...
1. When starting a new goal/session, call `dn_start_pipeline`.
2. When transitioning from one Phase to the next (e.g. from Phase 0 Interpret to Phase 1 Define), you MUST call `dn_advance_phase`.
3. If a step requires explicit human approval (e.g. locking the architecture), call `dn_request_gate_approval`.
...
Read `.dainexus/subagent-context/INTERPRETED_REQUEST.md` (from chat-interpreter Phase 0.A) for the authoritative request analysis. The chat-interpreter has already performed 9-dimension extraction and mode detection.

## Enhanced Mode Classification with Fuzzy Matching (v8.7+)

Detailed classification confidence levels, fuzzy trigger matching rules, trigger patterns, fallback chains, and planning/UX suggestions are documented in [references/mode-classification.md](skills/production-grade/references/mode-classification.md).
...
2. **Record attempt:** `bash scripts/dai-nexus-session-tracker.sh plan <score>`
3. **Check if gate needed:** `bash scripts/dai-nexus-session-tracker.sh check`
...
7. Append to `.dainexus/plan-lessons.md`
...
- This is NOT blocking — scope elicitation IS the DAI Nexus workflow
...
Detailed execution instructions, QA test sequences, and task flows for each non-Full-Build mode (e.g. Goal, Feature, Harden, Ship, Test, Review, Architect, Document, Explore, Research, Optimize, Design, Mobile, Game Build, XR Build, Analyze, Custom, Debug, AI Build, Migrate) are documented in [references/mode-execution.md](skills/production-grade/references/mode-execution.md).
...
2. **Mode Detection** — maps the request to DAI Nexus's 19 modes with confidence level (HIGH/MEDIUM/LOW)
...
All subsequent pipeline steps read `.dainexus/subagent-context/INTERPRETED_REQUEST.md` as the authoritative source of user intent — not the raw chat message.

## Tool-Specific Routing (from prompt-master)

Prompts, techniques, and template mappings for various AI code interfaces, reasoning engines, local LLMs, image generators, and autonomous agents are documented in [references/tool-routing.md](skills/production-grade/references/tool-routing.md).

## Auto-Initialization, Updates, and Session Lifecycle

System requirements, power level selection, auto-update mechanisms, session loading, memory retrieval, subagent context preparation, and subagent invocation protocols are documented in [references/initialization-and-lifecycle.md](skills/production-grade/references/initialization-and-lifecycle.md).
...
Write the choice to `.dainexus/settings.md`:
```markdown
# Pipeline Settings
...
Append to `.dainexus/settings.md`:
```markdown
...
Write analysis report to `.dainexus/scope-analysis.md` for future reference.
...
6. **Detect existing workspace & load memory** — if `.dainexus/` has prior state, use session-lifecycle resume protocol. If `.dainexus/session-log.json` has interrupted state, offer resume. Otherwise offer clean start via notify_user.
   - **Memory load:** Run `python scripts/lite/memory.py search "<project-name> <user-request-keywords>" --limit 5` to retrieve relevant project context. Inject results into your context for this session.
   - If no results or memory is empty, verify setup with `python scripts/lite/memory.py stats`.
   ...
   **Detect greenfield Full Build** (any of: Step 4 logged **Greenfield**; empty/minimal codebase with net-new product intent; user said "from scratch" / "new SaaS" / equivalent):
   ...
   - **Greenfield Full Build — BA is mandatory (no silent skip):**
     ...
     - **MUST** read `skills/business-analyst/SKILL.md` and run through at least **one full elicitation cycle** (stakeholder + structured questions per engagement depth: Express minimum **3** client-answered items, Standard **3–5**, Thorough **5+** with **2 rounds** if gaps remain) until:
       - `.dainexus/business-analyst/handoff/ba-package.md` exists **and**
...
Create a `task.md` file in `.dainexus/` with all 13 tasks and their statuses. Track dependencies and completion.

10. **Begin Phase 1** — read `phases/define.md` and start immediately. Do NOT ask "should I proceed?"
   - **Memory save (session start):** Run `python scripts/lite/memory.py add "Session started: [mode] mode for [brief request]. Engagement: [level]" --category session`
...
2. Run memory persistence commands:
   # Main architecture
   python scripts/lite/memory.py add "ARCH: [tech stack] | SERVICES: [service list] | REASON: [key rationale]" --category architecture
   
   # Individual ADRs
   python scripts/lite/memory.py add "DECISION: [ADR title] | ALTERNATIVE: [rejected options] | REASON: [why chosen]" --category decisions
   
   # Project scope
   python scripts/lite/memory.py add "PROJECT: [project name] | SCOPE: [feature list] | STATUS: active" --category project
...
A comprehensive table of common operational/architectural mistakes and their resolutions is documented in [references/common-mistakes.md](skills/production-grade/references/common-mistakes.md).
