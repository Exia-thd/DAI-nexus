...
Maintain per-session state in `.dainexus/session/asip-state.json`:
...
IF skill completes successfully:
  ...
  2. IF yes:
     ...
     - Save lesson to L2 graph: python scripts/lite/memory.py graph-add-node "lesson_[id]" "semantic" "Lesson" "Problem: ... | Research: ... | Lesson: ..."
     - Link to skill node: python scripts/lite/memory.py graph-link "lesson_[id]" "skill_[name]" --weight 1.5 --type "improves"
  3. Strengthen the path: python scripts/lite/memory.py graph-reinforce "current-session" "plan-quality" --factor 1.2
  4. Clear attempt counters for completed tasks
  5. Log: SKILL_COMPLETED_WITH_IMPROVEMENT
  6. Update .dainexus/asip-metrics.json
```
...
IF plan scores < 9.0:
  ...
  3. Decay edge in L2 graph: python scripts/lite/memory.py graph-decay "current-session" "plan-quality" --factor 0.5
...
IF execution fails with blocker:
  ...
  2. Record failure fact: python scripts/lite/memory.py add "Execution Blocker: [description] in skill [name]" --category blockers --importance 7
  3. Decay edge: python scripts/lite/memory.py graph-decay "current-session" "skill_[name]" --factor 0.5
...
│     - Append to .dainexus/plan-lessons.md OR execution-lessons.md │
│                                                                      │
...
2. Re-read .dainexus/plan-lessons.md OR execution-lessons.md
...
Update `.dainexus/asip-metrics.json`:
