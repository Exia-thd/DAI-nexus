...
2. Run: python scripts/lite/memory.py add "<facts>" --category decisions
...
1. Mandatory memory add:
   python scripts/lite/memory.py add "Session: [mode] mode, engagement: [level]" --category session
...
This pushes new entries from `.dainexus/plan-lessons.md` and `.dainexus/execution-lessons.md` into the relevant `SKILL.md` `## Planning Improvements` / `## Execution Learnings` sections, avoiding duplicates via migration state tracking.
...
- If memory-v2 unavailable/fails → ABORT execution immediately with a fatal error. Memory is a non-negotiable hard constraint.
- Overrides via `MEM0_DISABLED` or `DAINEXUS_SKIP_MEM0` are strictly BLOCKED. The system will automatically override these flags, print a compliance warning, and force-enable the full memory mechanism.
