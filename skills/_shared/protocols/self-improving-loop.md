...
│   The longer you use DAI Nexus, the SMARTER it becomes.     │
│                                                               │
...
| **Architectural** | DAI Nexus skills → Design patterns → Docs |
| **Tooling** | DAI Nexus protocols → Tool docs → Community |
...
1. In `.dainexus/execution-lessons.md`, append a cross-link:
...
2. The lesson migrator (`dai-nexus-lesson-migrator.sh`) extracts this and writes a stub to `.dainexus/plan-lessons.md`.
...
Each project using DAI Nexus develops:

1. **Project Profile** (`.dainexus/project-profile.json`)
...
```
✅ GOOD: Append to .dainexus/plan-lessons.md + skill SKILL.md
...
asip:
  ...
  adaptation:
    storeInProject: true
    skillAdaptationsDir: .dainexus/skill-adaptations
...
Output to `.dainexus/asip-metrics.json`:
