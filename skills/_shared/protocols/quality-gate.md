...
2. Score < threshold (default 9.0/10.0) → LEARN + RESEARCH + IMPROVE loop
   ...
   - Log lessons to .dainexus/plan-lessons.md
...
1. Existing tests still pass
   - Read baseline test count from .dainexus/project-profile.json
...
4. Convention compliance (brownfield only)
   - IF .dainexus/code-conventions.md exists:
...
1. Output maps to requirement
   - IF BRD exists (.dainexus/product-manager/BRD/):
...
3. Documentation exists
   - Workspace artifacts written to .dainexus/<skill>/
...
Write quality metrics to `.dainexus/quality-metrics.json` after each run:
