...
**Evidence-first note:** Every assumption about project history should be verified against memory before acting. If memory has a memory about a previous decision, cite it. If it contradicts the assumption, update the assumption.
...
Use this when MCP context offload is enabled and a tool result has been stored under `.dainexus/offload/<session>/`.
...
1. Open `.dainexus/offload/<session_id>/canvas.mmd` or run `trace-canvas`.
...
Scenario files live at `.dainexus/memory-bank/scenarios/*.md` and use this schema:
...
Persona memory lives at `.dainexus/memory-bank/persona.md` and uses this schema:
...
## Stable Preferences
- <durable user/project preference> [memory:<observation_id>]
...
`scripts/memory-retrieve.sh` loads persona/scenario layers before atom-level memory search. Tune caps with `MEM0_PERSONA_TOKENS`, `MEM0_SCENARIO_TOKENS`, `MEM0_SCENARIO_LIMIT`, and `MEM0_MAX_TOKENS`.
...
index = db.memory_index("dai-nexus project", limit=30)
...
Under the DAI Nexus Compliance Policy, memory is a **non-negotiable hard constraint** and cannot be disabled. 

If any attempt is made to bypass or disable memory (using `MEM0_DISABLED=true` or `DAINEXUS_SKIP_MEM0=1`), the system will **automatically override the bypass flag to false**, log an enforcement warning, and force-enable the full memory mechanism.
...
# Database location
MEM0_DB_PATH=.dainexus/memory.db
```

### Project Config (`memory.yaml`)

```yaml
# memory.yaml
project: my-project
database:
  path: .dainexus/memory.db
  wal_mode: true
...
## Integration with DAI Nexus Pipeline
...
│   └── SKILL.md              ← this file
├── scripts/
│   ├── scripts/lite/memory.py            ← PRIMARY CLI (SQLite + FTS5)
│   ├── scripts/lite/memory.py           ← DEPRECATED (TF-IDF + JSONL)
...
|--------|--------|-----------|
| **scripts/lite/memory.py** | DEPRECATED | Migrate to scripts/lite/memory.py |
| **local_memory.py** | DEPRECATED | Migrate to scripts/lite/memory.py |
