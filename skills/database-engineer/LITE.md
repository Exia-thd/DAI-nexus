...
3. ACTION (run EXPLAIN to verify scan type)   TARGET (scripts/explain.sql)   CHECK (psql -d dai-nexus -f scripts/explain.sql)
...
#### Step 3: Run EXPLAIN
- CHECK: `psql -d dai-nexus -f scripts/explain.sql` -> Plan updated: `Index Scan using Order_tenant_id_created_at_idx`.

### 5. VERIFY
CLAIM: tenant order query uses composite index and performs index scan
COMMAND: psql -d dai-nexus -c "EXPLAIN ANALYZE SELECT * FROM \"Order\" WHERE tenant_id = 't1' ORDER BY created_at DESC;"
