...
4. CACHE | Trigger passive idle checks and context offloading on token threshold hits | Confirm context volumes exceeding 1200 tokens are offloaded to `.dainexus/offload/` using trace handles.
...
```typescript
import { GeminiClient } from 'dai-nexus-gemini-sdk';
