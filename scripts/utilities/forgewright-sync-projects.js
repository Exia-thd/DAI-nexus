...
const REGISTRY_PATH = path.join(HOME, '.config/dai-nexus/registry.json');
const DAINEXUS_DIR = path.resolve(path.join(__dirname, '../..'));

console.log('==================================================');
console.log('🔄 DAI-NEXUS PROJECT SYNCER');
console.log('==================================================');
...
try {
  ...
  if (scanDir) {
    ...
    for (const file of files) {
      ...
      if (file.startsWith('.') || file.endsWith('.bfg-report') || file === 'dai-nexus') {
        continue;
      }
  ...
  for (const projectRoot of Object.keys(projects)) {
    ...
    try {
      ...
      // Copy CLAUDE.md
      const claudeMdSrc = path.join(DAINEXUS_DIR, 'CLAUDE.md');
      ...
      // Copy AGENTS.md
      const agentsMdSrc = path.join(DAINEXUS_DIR, 'AGENTS.md');
      ...
      for (const [dir, file] of copyPairs) {
        const src = path.join(DAINEXUS_DIR, dir, file);
    ...
    // 2. Clean up submodule / duplicate dai-nexus folder
    const localFwDir = path.join(projectRoot, 'dai-nexus');
    if (fs.existsSync(localFwDir)) {
      console.log(`  🧹 Found local DAI Nexus folder: ${localFwDir}`);
      try {
        ...
        if (isSubmodule) {
          ...
          try {
            execSync('git submodule deinit -f dai-nexus', { cwd: projectRoot, stdio: 'ignore' });
          } catch (e) {}
          try {
            execSync('git rm -f dai-nexus', { cwd: projectRoot, stdio: 'ignore' });
          } catch (e) {}
          
          // Delete .git/modules/dai-nexus if it exists
          const gitModuleDir = path.join(projectRoot, '.git', 'modules', 'dai-nexus');
        ...
        console.log(`  ✓ Cleaned up duplicate DAI Nexus folder`);
      } catch (err) {
        console.log(`  ⚠️ Failed to fully clean up dai-nexus folder: ${err.message}`);
      }
    }
    ...
    if (projects[projectRoot].dainexus_path !== DAINEXUS_DIR) {
      ...
      modified = true;
      console.log(`  ✓ Switched path to global DAI Nexus: ${DAINEXUS_DIR}`);
    }
  }
