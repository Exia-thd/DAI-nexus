#!/usr/bin/env bash
# =============================================================================
# DAI Nexus Install Script
# =============================================================================
# Install DAI Nexus with different profile configurations
#
# Usage:
#   dai-nexus install              # Interactive install
#   dai-nexus install --profile minimal   # Core pipeline only
#   dai-nexus install --profile core     # + security, QA
#   dai-nexus install --profile full     # Everything
#   dai-nexus install --dry-run           # Show what would be installed
# =============================================================================
...
show_help() {
    cat << EOF
${BOLD}dai-nexus install${NC}

Install DAI Nexus with customizable profiles.

${BOLD}USAGE${NC}
    dai-nexus install [options]
...
${BOLD}EXAMPLES${NC}
    dai-nexus install                    # Interactive install
    dai-nexus install --profile minimal  # Install minimal profile
    dai-nexus install --profile full    # Install everything
    dai-nexus install --dry-run         # Preview what would be installed

EOF
}

select_profile() {
    log_header "DAI Nexus Installation"
...
install_skills() {
    local profile="$1"
    local dry_run="$2"
    local source_dir="${DAINEXUS_SOURCE_DIR:-$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)}"
...
install_mcp() {
    ...
    local mcp_source="$source_dir/.dainexus/mcp-server"
    local mcp_dest="$DAINEXUS_DIR/mcp-server"
...
install_config() {
    ...
    local config_sources=(
        ...
        "CLAUDE.md"
        "AGENTS.md"
        ".dainexus/skills-config.json"
    )
    local config_dests=(
        "$DAINEXUS_DIR/.production-grade.yaml"
        "$DAINEXUS_DIR/CLAUDE.md"
        "$DAINEXUS_DIR/AGENTS.md"
...
install_hooks() {
    ...
    local gate_script="${DAINEXUS_DIR}/scripts/lite/stop-gate.sh"
    local gate_runtime_dir="${DAINEXUS_DIR}/scripts/lite"
...
var beforeGate = '${DAINEXUS_DIR}/scripts/lite/gemini-before-tool-gate.sh';
...
BT.push({ matcher: '*', hooks: [{ name: 'dai-nexus-policy', type: 'command', command: 'bash ' + beforeGate, timeout: 5000 }] });
...
cfg['dai-nexus-policy'] = {
  PreToolUse: [{
    matcher: '*',
    hooks: [{
      type: 'command',
      command: 'bash ${DAINEXUS_DIR}/scripts/lite/antigravity-pre-tool-gate.sh',
      timeout: 5
    }]
  }]
};
...
install_scripts() {
    ...
    local bin_dir="$DAINEXUS_DIR/bin"
    mkdir -p "$bin_dir"
...
create_alias() {
    ...
    if [[ -n "$shell_rc" ]]; then
        if ! grep -q "dai-nexus-lifecycle" "$shell_rc" 2>/dev/null; then
            echo "" >> "$shell_rc"
            echo "# DAI Nexus CLI" >> "$shell_rc"
            echo "$alias_line" >> "$shell_rc"
            echo "" >> "$shell_rc"
            echo "# Add dai-nexus to PATH" >> "$shell_rc"
            echo 'export PATH="'"$DAINEXUS_DIR"'/bin:\$PATH"' >> "$shell_rc"
...
# Create directories
if [[ "$DRY_RUN" == "false" ]]; then
    mkdir -p "$DAINEXUS_DIR" "$SKILLS_DIR" "$CONFIG_DIR"
fi
...
if [[ "$DRY_RUN" == "false" ]]; then
    ...
    echo "  Profile: $PROFILE"
    echo "  Location: $DAINEXUS_DIR"
    echo ""
    echo "  Installed skills: $(ls -1 "$SKILLS_DIR" 2>/dev/null | wc -l | tr -d ' ')"
    echo ""
    echo "  To use DAI Nexus, restart your terminal or run:"
    echo "    source ~/.bashrc   # or ~/.zshrc"
    echo ""
    echo "  Quick start:"
    echo "    dai-nexus doctor          # Check installation"
    echo "    dai-nexus list            # Show installed components"
    echo "    dai-nexus install --help  # See install options"
    echo ""
else
    echo ""
