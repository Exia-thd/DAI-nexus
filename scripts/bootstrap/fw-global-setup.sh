#!/usr/bin/env bash
# ─────────────────────────────────────────────────────────────────
# fw-global-setup — One-Time Global Setup for DAI Nexus MCP
#
# Installs DAI Nexus MCP globally so it works for ALL projects
...
#
# WHAT IT DOES:
#   1. Creates ~/.config/dai-nexus/ directory
...
WHAT GETS INSTALLED
    ~/.config/dai-nexus/
    ├── mcp-server/          Shared MCP server (from dai-nexus)
...
HOW IT WORKS
    ...
    the appropriate DAI Nexus MCP configuration. Works with any project
...
check_prerequisites() {
    ...
    # DAI Nexus
    if [[ ! -f "${DAINEXUS_DIR}/AGENTS.md" ]] && [[ ! -f "${DAINEXUS_DIR}/CLAUDE.md" ]]; then
        log_error "DAI Nexus not found at ${DAINEXUS_DIR}"
        log_info "Ensure you're running this from a dai-nexus directory"
        exit 1
    fi
    log_ok "DAI Nexus at ${DAINEXUS_DIR}"
}
...
install_mcp_server() {
    ...
    local source_server="${DAINEXUS_DIR}/skills/mcp-generator/templates"
    local source_mcp="${DAINEXUS_DIR}/mcp"
    ...
    # Create symlink to dai-nexus MCP server (avoids duplication)
...
install_global_launcher() {
    ...
    local source_launcher="${DAINEXUS_DIR}/scripts/fw-global-launcher.sh"
...
# DAI Nexus Global Settings
# Auto-generated by fw-global-setup.sh

# Global config directory
DAINEXUS_GLOBAL_CONFIG="${GLOBAL_CONFIG_DIR}"

# DAI Nexus installation path
DAINEXUS_DIR="${DAINEXUS_DIR}"

# Project registry
DAINEXUS_REGISTRY="${GLOBAL_REGISTRY}"
...
// Update dai-nexus to direct tsx execution (more reliable than bash launcher)
// Note: ${DAINEXUS_DIR} should point to the actual dai-nexus installation
cfg.mcpServers['dai-nexus'] = {
    command: 'npx',
    args: ['tsx', '${DAINEXUS_DIR}/mcp/src/index.ts'],
    env: {
...
// Update dai-nexus to direct tsx execution
cfg.mcpServers['dai-nexus'] = {
    command: 'npx',
    args: ['tsx', '${DAINEXUS_DIR}/mcp/src/index.ts']
...
cmd_check() {
    echo ""
    echo -e "${CYAN}━━━ DAI Nexus Global MCP Status ━━━${NC}"
    echo ""
    ...
    if [[ -f "$CURSOR_MCP_JSON" ]]; then
        ...
        elif grep -q "dai-nexus" "$CURSOR_MCP_JSON" 2>/dev/null; then
            log_warn "Cursor: $CURSOR_MCP_JSON (legacy - needs update)"
        else
            log_info "Cursor: $CURSOR_MCP_JSON (no dai-nexus)"
        fi
    else
    ...
    if [[ -f "$CLAUDE_CONFIG_JSON" ]]; then
        ...
        elif grep -q "dai-nexus" "$CLAUDE_CONFIG_JSON" 2>/dev/null; then
            log_warn "Claude Desktop: legacy launcher (needs update)"
        else
            log_info "Claude Desktop: $CLAUDE_CONFIG_JSON (no dai-nexus)"
        fi
    else
...
cmd_uninstall() {
    echo ""
    echo -e "${YELLOW}━━━ Uninstalling DAI Nexus Global Setup ━━━${NC}"
    echo ""
...
cmd_diagnose() {
    echo ""
    echo -e "${CYAN}━━━ DAI Nexus Global MCP Diagnostics ━━━${NC}"
    echo ""
    
    log_step "Environment"
    echo "  PWD:              $(pwd)"
    echo "  DAINEXUS_DIR:  ${DAINEXUS_DIR}"
    echo "  HOME:             ${HOME}"
...
main() {
    ...
    echo ""
    echo -e "${CYAN}⚡ DAI Nexus Global MCP Setup${NC}"
    ...
    case "$mode" in
        ...
        install)
            ...
            if [[ ! -f "${DAINEXUS_DIR}/scripts/fw-global-launcher.sh" ]]; then
