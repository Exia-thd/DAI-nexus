#!/usr/bin/env bash
# ═══════════════════════════════════════════════════════════════════════════════
# DAI Nexus Installer — One-Command Setup for Fresh or Existing Projects
#
# SCENARIOS HANDLED:
...
VERSION="1.0.0"
GITHUB_REPO="https://github.com/Exia-thd/DAI-nexus"
DAINEXUS_DEFAULT="${HOME}/Documents/GitHub/dai-nexus"
...
jsonc_parser_module() {
    ...
    for candidate in \
        "$HOME/.dainexus/mcp-server/node_modules/jsonc-parser" \
...
configured = any(
    ...
    and entry.get("command") == str(__import__('pathlib').Path.home() / '.dainexus' / 'mcp-server' / 'node_modules' / '.bin' / 'tsx')
    and entry.get("args") == [str(__import__('pathlib').Path.home() / '.dainexus' / 'mcp-server' / 'src' / 'index.ts')]
...
PY
    ...
    if ! bash -c 'source "$1"; jsonc_self_contained verify "$2" "" "$3" dai-nexus "$4" "$5"' \
        ...
        "$HOME/.dainexus/mcp-server/src/index.ts" >/dev/null 2>&1; then
        return 1
    fi
...
install_dai-nexus() {
    ...
    if [[ -d "${DAINEXUS_DEFAULT}" ]]; then
        warn "DAI Nexus already exists at ${DAINEXUS_DEFAULT}"
        ...
        mkdir -p "$(dirname "${DAINEXUS_DEFAULT}")"
        git clone --recursive "$GITHUB_REPO" "${DAINEXUS_DEFAULT}"
        cd "${DAINEXUS_DEFAULT}"
    fi
...
ensure_dai-nexus() {
    ...
    if [[ -z "$fw_dir" ]] || [[ ! -d "$fw_dir" ]]; then
        warn "DAI Nexus not found!"
        read -p "  Install DAI Nexus now? [Y/n] " -n 1 -r
        echo
        if [[ $REPLY =~ ^[Nn]$ ]]; then
...
build_mcp_server() {
    ...
    info "Building DAI Nexus MCP Server..."
    ...
    if npm run build 2>&1 | tail -5; then
        success "DAI Nexus MCP Server built"
    else
...
copy_project_files() {
    ...
    case "$policy_result" in
        created:*) success "Seeded .dainexus/execution-policy.yaml" ;;
        preserved:*) info "Preserved existing .dainexus/execution-policy.yaml" ;;
...
cmd_check() {
    ...
    echo ""
    echo -e "${BOLD}${CYAN}━━━ DAI Nexus Status ━━━${NC}"
    echo ""
...
cmd_diagnose() {
    ...
    info "DAI Nexus:"
    echo "  Searched:   ${DAINEXUS_DEFAULT}"
    ...
    for f in \
        ...
        "$HOME/.dainexus/mcp-server/src/index.ts" \
...
OPTIONS:
  ...
  --install-fw    Install DAI Nexus first
...
main() {
    ...
    echo ""
    echo -e "${BOLD}${CYAN}╔══════════════════════════════════════════════════════════════╗${NC}"
    echo -e "${BOLD}${CYAN}║${NC}  ${BOLD}DAI Nexus Setup v${VERSION}${NC}                                       ${BOLD}${CYAN}║${NC}"
    echo -e "${BOLD}${CYAN}╚══════════════════════════════════════════════════════════════╝${NC}"
    echo ""
