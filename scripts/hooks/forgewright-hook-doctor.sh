...
resolve_python311() {
    ...
    if [[ -n "${DAINEXUS_PYTHON_BIN:-}" && -x "${DAINEXUS_PYTHON_BIN}" ]]; then
        if "${DAINEXUS_PYTHON_BIN}" -c 'import sys; raise SystemExit(0 if sys.version_info >= (3, 11) else 1)' >/dev/null 2>&1; then
            printf '%s\n' "${DAINEXUS_PYTHON_BIN}"
            return 0
        fi
    fi
...
echo ""
echo -e "${CYAN}⚕️  DAI Nexus Hook Doctor${NC}"
echo ""
echo "  Hook Script: $HOOK_SCRIPT"
echo "  Memory Session: $MEMORY_SESSION"
echo "  Profile: ${DAINEXUS_HOOK_PROFILE:-standard}"
echo ""
...
# DAI Nexus directories
for dir in \
    "${HOME}/.dainexus" \
    "${HOME}/.dainexus/sessions" \
    "${SCRIPT_DIR}/../.." \
    "${HOME}/.claude" \
    "${HOME}/.gemini" \
    "${HOME}/.cursor" \
...
# Profile
if [[ -n "${DAINEXUS_HOOK_PROFILE:-}" ]]; then
    case "$DAINEXUS_HOOK_PROFILE" in
        minimal|standard|strict)
            log_pass "Profile set: $DAINEXUS_HOOK_PROFILE"
            ;;
        *)
            log_warn "Profile invalid: $DAINEXUS_HOOK_PROFILE (expected: minimal|standard|strict)"
            ;;
    esac
else
...
# Tick interval
if [[ -n "${DAINEXUS_MEMORY_TICK_INTERVAL:-}" ]]; then
    log_pass "Tick interval: $DAINEXUS_MEMORY_TICK_INTERVAL"
...
# Disabled hooks
if [[ -n "${DAINEXUS_DISABLED_HOOKS:-}" ]]; then
    log_pass "Disabled hooks: $DAINEXUS_DISABLED_HOOKS"
else
    log_info "No disabled hooks"
fi

# Session start max chars
if [[ -n "${DAINEXUS_SESSION_START_MAX_CHARS:-}" ]]; then
    log_pass "Session start max chars: $DAINEXUS_SESSION_START_MAX_CHARS"
else
...
if [[ -f "$GEMINI_SETTINGS" ]]; then
    ...
    gemini_schema=$(node -e "try { var c=JSON.parse(require('fs').readFileSync('$GEMINI_SETTINGS')); var afterOk=c.hooks && Array.isArray(c.hooks.AfterAgent) && c.hooks.AfterAgent.some(function(g){ return Array.isArray(g.hooks) && g.hooks.some(function(h){ return h.type === 'command' && typeof h.command === 'string' && h.command.includes('stop-gate.sh --platform GEMINI'); }); }); var beforeOk=c.hooks && Array.isArray(c.hooks.BeforeTool) && c.hooks.BeforeTool.some(function(g){ return g.matcher === '*' && Array.isArray(g.hooks) && g.hooks.some(function(h){ return h.name === 'dai-nexus-policy' && h.type === 'command' && typeof h.command === 'string' && h.command.includes('gemini-before-tool-gate.sh') && typeof h.timeout === 'number'; }); }); console.log(Boolean(afterOk && beforeOk)); } catch (_) { console.log(false); }" 2>/dev/null || echo "false")
...
BT.push({ matcher: '*', hooks: [{ name: 'dai-nexus-policy', type: 'command', command: 'bash $PROJECT_ROOT/scripts/lite/gemini-before-tool-gate.sh', timeout: 5000 }] });
...
cfg['dai-nexus-policy'] = { PreToolUse: [{ matcher: '*', hooks: [{ type: 'command', command: 'bash $ANTIGRAVITY_GATE', timeout: 5 }] }] };
...
if [[ -f "$ANTIGRAVITY_SETTINGS" ]]; then
    ...
    if [[ "$antigravity_always_proceed" == "true" ]]; then
        log_warn "Antigravity toolPermission is always-proceed; DAI Nexus still gates tools, but native permission prompts are disabled"
    fi
fi
...
if [[ -z "$SUPERPROJECT_ROOT" ]]; then
    log_info "DAI Nexus is not running as a submodule; no parent policy to seed"
else
    PARENT_POLICY="${SUPERPROJECT_ROOT}/.dainexus/execution-policy.yaml"
...
if [[ "$QUICK_MODE" == "false" ]]; then
    ...
    init_file="${HOME}/.dainexus/hooks-initialized-standard"
    if [[ -f "$init_file" ]]; then
...
current_profile="${DAINEXUS_HOOK_PROFILE:-standard}"
