#!/usr/bin/env bash
#────────────────────────────────────────────────────────────────────────────
# DAI Nexus Memory Hook Integration
#────────────────────────────────────────────────────────────────────────────
...
#
# Profile System:
#   DAINEXUS_HOOK_PROFILE=minimal|standard|strict (default: standard)
#   DAINEXUS_DISABLED_HOOKS=hook1,hook2 (comma-separated)
#   DAINEXUS_SESSION_START_MAX_CHARS=50000 (default)
#   DAINEXUS_MEMORY_TICK_INTERVAL=3 (messages between ticks)
#
# Profiles:
...
export DAINEXUS_HOOK_PROFILE="${DAINEXUS_HOOK_PROFILE:-standard}"

# Disabled hooks: comma-separated list (e.g., "checkpoint,verbose")
export DAINEXUS_DISABLED_HOOKS="${DAINEXUS_DISABLED_HOOKS:-}"

# Session start max chars for context loading
export DAINEXUS_SESSION_START_MAX_CHARS="${DAINEXUS_SESSION_START_MAX_CHARS:-50000}"

# Tick interval (messages between ticks) - backward compatible default
export DAINEXUS_MEMORY_TICK_INTERVAL="${DAINEXUS_MEMORY_TICK_INTERVAL:-3}"

# Legacy env var support (backward compatible)
export MEMORY_CHECKPOINT_INTERVAL="${MEMORY_CHECKPOINT_INTERVAL:-$DAINEXUS_MEMORY_TICK_INTERVAL}"
export MEMORY_TOKEN_THRESHOLD="${MEMORY_TOKEN_THRESHOLD:-70}"

# Verbose mode (enabled in strict profile)
DAINEXUS_VERBOSE="${DAINEXUS_VERBOSE:-false}"

# ─── Logging Functions ────────────────────────────────────────────────────────

log_debug() {
    if [[ "$DAINEXUS_HOOK_PROFILE" == "strict" ]] || [[ "$DAINEXUS_VERBOSE" == "true" ]]; then
        echo -e "${BLUE}[fw-hook]${NC} DEBUG: $1" >&2
    fi
}

log_info() {
    if [[ "$DAINEXUS_HOOK_PROFILE" != "minimal" ]]; then
...
get_profile_hooks() {
    local profile="${DAINEXUS_HOOK_PROFILE:-standard}"
    case "$profile" in
        minimal)
            echo "tick"
            ;;
...
HOOKS_INITIALIZED="${HOME}/.dainexus/hooks-initialized-${DAINEXUS_HOOK_PROFILE}"
LAST_TOKEN_CHECK_FILE="${HOME}/.dainexus/last-token-check"
...
is_hook_enabled() {
    ...
    if [[ -n "$DAINEXUS_DISABLED_HOOKS" ]]; then
        IFS=',' read -ra DISABLED <<< "$DAINEXUS_DISABLED_HOOKS"
        for disabled in "${DISABLED[@]}"; do
            # Trim whitespace
            disabled=$(echo "$disabled" | xargs)
            if [[ "$hook_name" == "$disabled" ]]; then
                log_debug "Hook '$hook_name' is disabled by DAINEXUS_DISABLED_HOOKS"
                return 1
            fi
        done
    fi
...
check_token_threshold() {
    ...
    if [[ "$DAINEXUS_HOOK_PROFILE" == "strict" ]]; then
...
main() {
    ...
    if [[ ! -f "$HOOKS_INITIALIZED" ]]; then
        log_debug "Initializing hooks for profile: $DAINEXUS_HOOK_PROFILE"
    ...
    case "${action}" in
        ...
        profile)
            ...
            echo "Profile: $DAINEXUS_HOOK_PROFILE"
            echo "Disabled hooks: ${DAINEXUS_DISABLED_HOOKS:-none}"
            echo "Tick interval: $DAINEXUS_MEMORY_TICK_INTERVAL"
            echo "Session start max chars: $DAINEXUS_SESSION_START_MAX_CHARS"
            ;;
