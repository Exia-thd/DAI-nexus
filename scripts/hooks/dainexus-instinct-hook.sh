#!/usr/bin/env bash
#────────────────────────────────────────────────────────────────────────────
# DAI Nexus Instinct Hook Integration
#────────────────────────────────────────────────────────────────────────────
...
if [[ "${DAINEXUS_INSTINCTS_ENABLED:-1}" == "0" ]]; then
    exit 0
fi

# ─── Instincts Config ──────────────────────────────────────────────

# Determine paths based on whether we're in a project or dai-nexus install
WORKSPACE_ROOT="${DAINEXUS_WORKSPACE:-$PWD}"
DAINEXUS_INSTALL="${HOME}/.dainexus"

# Case 1: Project workspace has .dainexus/instincts
if [[ -d "${WORKSPACE_ROOT}/.dainexus/instincts" ]]; then
    INSTINCTS_DIR="${WORKSPACE_ROOT}/.dainexus/instincts"
# Case 2: DAI Nexus install has instincts at ~/.dainexus/instincts
elif [[ -d "${DAINEXUS_INSTALL}/instincts" ]]; then
    INSTINCTS_DIR="${DAINEXUS_INSTALL}/instincts"
# Case 3: Default to workspace .dainexus
else
    INSTINCTS_DIR="${WORKSPACE_ROOT}/.dainexus/instincts"
fi

STORE_PATH="${DAINEXUS_INSTINCTS_STORE:-$INSTINCTS_DIR/store.json}"
NODE_PATH="${DAINEXUS_DIR}/node_modules"
...
main() {
    ...
    case "${action}" in
        observe)
            ...
            local session_id="${DAINEXUS_SESSION_ID:-default}"
            local project_root="${DAINEXUS_WORKSPACE:-$PWD}"
...
show_status() {
    ...
    echo ""
    echo "Enabled: ${DAINEXUS_INSTINCTS_ENABLED:-1}"
