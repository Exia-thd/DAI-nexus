...
log_debug() {
    if [[ "$DEBUG" == "1" ]]; then
        echo "[dai-nexus-launcher DEBUG] $*" >&2
    fi
}

log_error() {
    echo "[dai-nexus-launcher ERROR] $*" >&2
}

log_info() {
    echo "[dai-nexus-launcher] $*" >&2
}

# ─── Constants ─────────────────────────────────────────────────

DAINEXUS_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd -P)"
DAINEXUS_DIR="${DAINEXUS_DIR%/scripts}"
CANONICAL_SERVER_PATH="$HOME/.dainexus/mcp-server/src/index.ts"
CANONICAL_TSX_PATH="$HOME/.dainexus/mcp-server/node_modules/.bin/tsx"
...
resolve_workspace() {
    ...
    if [[ -n "${DAINEXUS_WORKSPACE:-}" ]]; then
        ...
        log_debug "  → Using DAINEXUS_WORKSPACE: $workspace"
...
find_manifest() {
    ...
    # Check .dainexus/ (legacy/project-level)
    if [[ -f "$workspace/.dainexus/mcp-manifest.json" ]]; then
        log_debug "  → Found: $workspace/.dainexus/mcp-manifest.json"
        echo "$workspace/.dainexus/mcp-manifest.json"
        return 0
    fi
...
find_dai-nexus() {
    ...
    # Pattern 1: Use DAINEXUS_DIR (from script location)
    if [[ -d "${DAINEXUS_DIR}" ]]; then
        log_debug "  → Using DAINEXUS_DIR: $DAINEXUS_DIR"
        echo "$DAINEXUS_DIR"
        return 0
    fi

    # Pattern 2: .dainexus/ directory in workspace
    if [[ -d "$workspace/.dainexus" ]]; then
        log_debug "  → Found .dainexus in workspace"
        echo "$workspace/.dainexus"
        return 0
    fi

    # Pattern 3: dai-nexus/ submodule
    if [[ -d "$workspace/dai-nexus" ]]; then
        log_debug "  → Found dai-nexus/ submodule"
        echo "$workspace/dai-nexus"
        return 0
    fi

    # Pattern 4: Look up directory tree for dai-nexus
    local current="$workspace"
    while [[ "$current" != "/" ]]; do
        if [[ -d "$current/.dainexus" ]]; then
            log_debug "  → Found .dainexus at $current"
            echo "$current/.dainexus"
            return 0
        fi
        if [[ -f "$current/AGENTS.md" ]] || [[ -f "$current/CLAUDE.md" ]]; then
            # This is the dai-nexus project itself
            log_debug "  → Workspace IS the dai-nexus project"
            echo "$current"
            return 0
        fi
...
resolve_server_cmd() {
    ...
    if [[ -n "$manifest" ]] && [[ -f "$manifest" ]]; then
        ...
        # Find dai-nexus-mcp-server from manifest
        local server_path
...
try {
  ...
  var server = (m.servers || []).find(function(s){
    return (s.type === 'dai-nexus-mcp-server' || s.name === 'dai-nexus') && s.enabled !== false && s.auto_start !== false;
  });
  if (server) {
...
NODE
        ...
        if [[ -n "$server_path" ]] && [[ -f "$server_path" ]]; then
            log_debug "  → Found dai-nexus-mcp-server: $server_path"
...
main() {
    local workspace manifest dai-nexus server_cmd

    log_debug "=== DAI Nexus MCP Launcher Started ==="
    log_debug "DAINEXUS_WORKSPACE: ${DAINEXUS_WORKSPACE:-<not set>}"
    log_debug "MCP_WORKSPACE_ROOT: ${MCP_WORKSPACE_ROOT:-<not set>}"
    log_debug "PWD: $(pwd)"
    log_debug "DAINEXUS_DIR: $DAINEXUS_DIR"
