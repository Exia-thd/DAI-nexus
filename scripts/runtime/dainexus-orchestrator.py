...
def resolve_model(environ: dict[str, str] | None = None) -> str:
    ...
    model = _first_nonempty(env.get("DAINEXUS_MODEL"), env.get("NINEROUTER_MODEL"))
    if not model:
        raise ValueError(
            "DAINEXUS_MODEL is required (NINEROUTER_MODEL is a legacy fallback)"
        )
    return model
...
def resolve_api_url(environ: dict[str, str] | None = None) -> str:
    env = os.environ if environ is None else environ
    exact_url = _first_nonempty(env.get("DAINEXUS_API_URL"))
    if exact_url:
        return exact_url
    ...
    raise ValueError(
        "DAINEXUS_API_URL or NINEROUTER_BASE_URL must be explicitly configured"
    )
...
class RuntimeLimits:
    ...
    def from_env(cls, environ: dict[str, str] | None = None) -> "RuntimeLimits":
        ...
        return cls(
            ...
            max_turns=_optional_positive_int(env, "DAINEXUS_MAX_TURNS"),
            max_tool_calls_per_turn=_positive_int(
                env, "DAINEXUS_MAX_TOOL_CALLS_PER_TURN", 8
            ),
            max_tool_calls_total=_positive_int(
                env, "DAINEXUS_MAX_TOOL_CALLS_TOTAL", 40
            ),
            max_output_tokens=_positive_int(env, "DAINEXUS_MAX_OUTPUT_TOKENS", 8192),
            max_http_response_bytes=_positive_int(
                env, "DAINEXUS_MAX_HTTP_RESPONSE_BYTES", 2_000_000
            ),
            max_tool_argument_bytes=_positive_int(
                env, "DAINEXUS_MAX_TOOL_ARGUMENT_BYTES", 100_000
            ),
            max_tool_result_bytes=_positive_int(
                env, "DAINEXUS_MAX_TOOL_RESULT_BYTES", 250_000
            ),
            max_context_bytes=_positive_int(
                env, "DAINEXUS_MAX_CONTEXT_BYTES", 1_000_000
            ),
            max_tools=_positive_int(env, "DAINEXUS_MAX_TOOLS", 256),
...
_MODEL = _first_nonempty(
    os.environ.get("DAINEXUS_MODEL"), os.environ.get("NINEROUTER_MODEL")
)
...
class DaiNexusAgent:
    ...
    def _call_api(self, tools: List[Dict]) -> Dict:
        ...
        if not _BASE_URL:
            print("[!] DAINEXUS_API_URL or NINEROUTER_BASE_URL env var is not set.")
            sys.exit(1)
        if not _API_KEY:
            print("[!] DAINEXUS_API_KEY or NINEROUTER_API_KEY env var is not set.")
            sys.exit(1)
        if not _MODEL:
            print("[!] DAINEXUS_MODEL env var is not set.")
            sys.exit(1)
        headers = {
    ...
    async def run(self, task: str):
        ...
        if os.path.exists(
            ...
            os.path.join(self.code_dir, "..", ".dainexus", "mcp-manifest.json")
        ):
            ...
            mcp_servers.append(
                {
                    ...
                    "params": StdioServerParameters(
                        command="bash",
                        args=[mcp_launcher],
                        env={**os.environ, "DAINEXUS_WORKSPACE": self.code_dir},
                    ),
                }
            )
        ...
        profile_path = os.path.join(
            self.code_dir, "..", ".dainexus", "project-profile.json"
        )
        ...
        use_lite = os.environ.get("DAINEXUS_LITE", "false").lower() == "true"
