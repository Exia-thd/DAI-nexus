#!/usr/bin/env bash
# ═══════════════════════════════════════════════════════════════════════════════
# DAI Nexus Batch Submodule Updater
# ═══════════════════════════════════════════════════════════════════════════════
...
for arg in "$@"; do
    case "$arg" in
        ...
        --help|-h)
            ...
            echo ""
            echo "  --pull           Auto-pull DAI Nexus updates in all projects"
...
if $INSTALL_HOOKS; then
    ...
    for scan_dir in $SCAN_DIRS; do
        ...
        done < <(find "$scan_dir" -maxdepth 3 -name ".gitmodules" -exec grep -l "dai-nexus" {} \; 2>/dev/null)
    done

    echo ""
    success "Installed hooks in $count projects."
    info "After git pull or checkout, each project will auto-check DAI Nexus and refresh its installed runtime."
    exit 0
fi
...
# Batch update mode
# ═══════════════════════════════════════════════════════════════════════════════
header "DAI Nexus Batch Submodule Updater"
...
# Get latest DAI Nexus commit for reference
FW_LATEST=""
if [[ -d "$HOME/GitHub/dai-nexus/.git" ]]; then
    FW_LATEST=$(cd "$HOME/GitHub/dai-nexus" && git rev-parse --short HEAD 2>/dev/null || echo "unknown")
    info "DAI Nexus latest: ${GREEN}$FW_LATEST${NC}"
fi
    ...
    while IFS= read -r gitmodules; do
        ...
        submodule_dir=$(
            ...
            while IFS= read -r path_key; do
                ...
                if printf '%s' "$path_value" | grep -qi dai-nexus; then
                    printf '%s\n' "$path_value"
                    break
                fi
            done
        ...
        if [[ -z "$submodule_dir" ]]; then
            submodule_dir=$(
                ...
                while IFS= read -r url_key; do
                    ...
                    if printf '%s' "$url_value" | grep -qi dai-nexus; then
        ...
        [[ -z "$submodule_dir" ]] && submodule_dir="dai-nexus"
        ...
        if [[ "$behind" -gt 0 ]]; then
            ...
            if $DO_PULL; then
                ...
                if $DO_COMMIT; then
                    ...
                    git commit -m "chore: update dai-nexus submodule to $new_sha" --no-verify >/dev/null 2>&1 || true
