#!/usr/bin/env bash
# =============================================================================
# DAI Nexus Lifecycle Script
# =============================================================================
# Manage DAI Nexus installation lifecycle
#
# Usage:
#   dai-nexus doctor              # Diagnose issues
#   dai-nexus doctor --fix       # Auto-fix common issues
#   dai-nexus list               # Show installed components
#   dai-nexus uninstall          # Remove all configs
#   dai-nexus uninstall --purge  # Remove all + data
# =============================================================================
...
# DAI Nexus directory
DAINEXUS_DIR="${DAINEXUS_DIR:-$HOME/.dainexus}"
SKILLS_DIR="$DAINEXUS_DIR/skills"
MCP_DIR="$DAINEXUS_DIR/mcp-server"
CONFIG_DIR="$DAINEXUS_DIR/config"
...
${BOLD}USAGE${NC}
    ...
    dai-nexus uninstall          # Remove all configs
    dai-nexus uninstall --purge  # Remove all + data
    dai-nexus status             # Show health status
...
${BOLD}COMMANDS${NC}
    ...
    uninstall           Remove DAI Nexus configurations (keeps data)
    uninstall --purge   Remove everything including all data

${BOLD}EXAMPLES${NC}
    dai-nexus doctor              # Check health
    dai-nexus doctor --fix        # Auto-fix issues
    dai-nexus uninstall --purge   # Complete removal
...
check_directory() {
    log_header "Checking DAI Nexus Directory"

    if [[ -d "$DAINEXUS_DIR" ]]; then
        log_success "DAI Nexus directory exists: $DAINEXUS_DIR"
    else
        log_error "DAI Nexus directory not found: $DAINEXUS_DIR"
        log_info "Run: dai-nexus install"
    fi
}
...
check_mcp() {
    ...
    local mcp_configs=(
        ...
        "$HOME/.config/Code/User/globalStorage/Exia-thd.mcp-server/config.json"
    )

    for config in "${mcp_configs[@]}"; do
        if [[ -f "$config" ]]; then
            if grep -q "dai-nexus" "$config" 2>/dev/null; then
                log_success "  ✓ $(basename "$config")"
            else
                log_warn "  ~ $(basename "$config") (exists but no dai-nexus config)"
            fi
        fi
    done
}
...
check_permissions() {
    ...
    if [[ -d "$DAINEXUS_DIR" ]]; then
        if [[ -w "$DAINEXUS_DIR" ]]; then
            log_success "DAI Nexus directory is writable"
        else
            log_error "DAI Nexus directory is not writable"
        fi
    fi
}
...
fix_common_issues() {
    ...
    if [[ ! -d "$DAINEXUS_DIR" ]]; then
        log_fix "Creating DAI Nexus directory"
        mkdir -p "$DAINEXUS_DIR"
    fi

    # Fix 2: Ensure bin directory exists
    if [[ ! -d "$DAINEXUS_DIR/bin" ]]; then
        log_fix "Creating bin directory"
        mkdir -p "$DAINEXUS_DIR/bin"
    fi

    # Fix 3: Create hooks symlink if missing
    if [[ -f "$DAINEXUS_DIR/.claude/settings.json" ]]; then
        if [[ ! -f "$HOME/.claude/settings.json" ]]; then
            log_fix "Creating Claude Code settings.json symlink"
            mkdir -p "$HOME/.claude"
            ln -sf "$DAINEXUS_DIR/.claude/settings.json" "$HOME/.claude/settings.json"
        fi
    fi
    if [[ -f "$DAINEXUS_DIR/.gemini/settings.json" ]]; then
        if [[ ! -f "$HOME/.gemini/settings.json" ]]; then
            log_fix "Creating Gemini settings.json symlink"
            mkdir -p "$HOME/.gemini"
            ln -sf "$DAINEXUS_DIR/.gemini/settings.json" "$HOME/.gemini/settings.json"
        fi
    fi
    if [[ -f "$DAINEXUS_DIR/.cursor/hooks.json" ]]; then
        if [[ ! -f "$HOME/.cursor/hooks.json" ]]; then
            log_fix "Creating Cursor hooks.json symlink"
            mkdir -p "$HOME/.cursor"
            ln -sf "$DAINEXUS_DIR/.cursor/hooks.json" "$HOME/.cursor/hooks.json"
        fi
    fi
    if [[ -f "$DAINEXUS_DIR/.codex/config.toml" ]]; then
        if [[ ! -f "$HOME/.codex/config.toml" ]]; then
            log_fix "Creating Codex config.toml symlink"
            mkdir -p "$HOME/.codex"
            ln -sf "$DAINEXUS_DIR/.codex/config.toml" "$HOME/.codex/config.toml"
        fi
    fi
    ...
    if [[ -d "$DAINEXUS_DIR/bin" ]]; then
        ...
        chmod +x "$DAINEXUS_DIR/bin"/*.sh 2>/dev/null || true
        chmod +x "$DAINEXUS_DIR/bin"/*.py 2>/dev/null || true
    fi
...
list_all() {
    ...
    echo ""
    echo -e "  ${BOLD}Location:${NC} $DAINEXUS_DIR"
    echo ""
    ...
    if [[ -d "$DAINEXUS_DIR/bin" ]]; then
        for script in "$DAINEXUS_DIR/bin"/*.sh "$DAINEXUS_DIR/bin"/*.py; do
            if [[ -f "$script" ]]; then
...
uninstall() {
    ...
    log_header "Uninstall DAI Nexus"
    ...
    for config in "${mcp_configs[@]}"; do
        if [[ -f "$config" ]]; then
            ...
            # Remove dai-nexus entries
            if command -v python3 &> /dev/null; then
                python3 -c "
import json
with open('$config', 'r') as f:
    data = json.load(f)
if 'mcpServers' in data and 'dai-nexus' in data['mcpServers']:
    del data['mcpServers']['dai-nexus']
...
" 2>/dev/null || log_warn "Could not clean $config"
    ...
    for rc in "$HOME/.bashrc" "$HOME/.zshrc"; do
        if [[ -f "$rc" ]]; then
            if grep -q "dai-nexus" "$rc" 2>/dev/null; then
                # Remove dai-nexus lines
                grep -v "dai-nexus\|DAI Nexus CLI" "$rc" > "$rc.tmp" 2>/dev/null || true
                mv "$rc.tmp" "$rc"
    ...
    # Remove DAI Nexus directory
    log_info "Removing DAI Nexus directory..."
    if [[ -d "$DAINEXUS_DIR" ]]; then
        if [[ "$purge" == "true" ]]; then
            rm -rf "$DAINEXUS_DIR"
            log_success "Removed: $DAINEXUS_DIR"
        else
            # Only remove certain subdirectories
            rm -rf "$DAINEXUS_DIR/bin" 2>/dev/null || true
            rm -rf "$DAINEXUS_DIR/mcp-server" 2>/dev/null || true
    ...
    if [[ "$purge" == "true" ]]; then
        echo ""
        echo "  All DAI Nexus data has been removed."
        echo "  To reinstall, run: dai-nexus install"
    else
        echo ""
        echo "  Configuration removed. To fully reinstall, run:"
        echo "    dai-nexus install"
    fi
}
...
show_status() {
    ...
    echo ""
    echo -e "  ${BOLD}DAI Nexus Status${NC}"
    echo ""
    ...
    if [[ "$health" == "healthy" ]]; then
        ...
        echo ""
        echo "  Run 'dai-nexus doctor' for details"
    else
        echo -e "  ${RED}✗${NC} Installation issues detected"
        echo ""
        echo "  Run 'dai-nexus install' to fix"
    fi
...
case "$COMMAND" in
    doctor|diagnose)
        ...
        if [[ "$ISSUES" -eq 0 && "$WARNINGS" -eq 0 ]]; then
            ...
            echo ""
            echo "  Run 'dai-nexus doctor --fix' to auto-fix"
        fi
        echo ""
        ;;
