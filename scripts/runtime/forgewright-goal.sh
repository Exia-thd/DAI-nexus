...
#   dai-nexus goal status
#   dai-nexus goal clear
...
cmd_set() {
    ...
    if [ -z "$condition" ]; then
        log_error "Usage: dai-nexus goal set <condition>"
        log_info "Example: dai-nexus goal set 'All tests pass and lint is clean'"
        exit 1
    fi
...
EOF
    ...
    echo ""
    log_info "DAI Nexus will now work continuously toward this goal."
    log_info "Use 'dai-nexus goal status' to check progress."
    log_info "Use 'dai-nexus goal clear' to cancel."
}
...
cmd_status() {
    if [ ! -f "$GOAL_FILE" ]; then
        ...
        echo ""
        log_info "Set a goal with: dai-nexus goal set <condition>"
        exit 0
    fi
...
case "$COMMAND" in
    ...
    continue)
        ...
        ;;
    help|--help|-h)
        echo "dai-nexus goal - Goal-driven workflow"
        echo ""
        echo "Usage:"
        echo "  dai-nexus goal set <condition>   Set a goal"
        echo "  dai-nexus goal status            Check goal status"
        echo "  dai-nexus goal clear             Clear the active goal"
        echo ""
        echo "Examples:"
        echo "  dai-nexus goal set \"All tests pass\""
        echo "  dai-nexus goal set \"Build succeeds and lint is clean\""
        ;;
    *)
        if [ -n "$COMMAND" ]; then
            log_error "Unknown command: $COMMAND"
            echo "Use 'dai-nexus goal help' for usage"
            exit 1
        fi
        cmd_status
        ;;
