...
# DAI Nexus turn-completion verify-gate.
...
if decision == "continue":
    ...
    print(json.dumps({"decision": "block", "reason": reason or "DAI Nexus verify gate blocked stop."}))
PYEOF
}
...
gate_block() {
  local reason="${1:-DAI Nexus verify gate blocked stop.}"
  if [[ "$CODEX_HOOK" -eq 1 ]]; then
...
if [[ -n "$TURN_FROM_PAYLOAD" ]]; then
  export DAINEXUS_TURN="$TURN_FROM_PAYLOAD"
fi
...
_SKIP_PATTERNS=(
  ...
  ".dainexus/*" "memory.db*" ".gitnexus/*" ".dainexus-node/*"
)
...
if [[ $GATE_RC -ne 0 ]]; then
  ...
  gate_block "DAI Nexus evidence validation failed. Review hook stderr for details."
fi
...
if ! printf '%s' "$RESPONSE_CONTENT" | python3 "${SCRIPT_DIR}/rule-validator.py" --runtime >/dev/null; then
  ...
  gate_block "DAI Nexus strict VERIFY validation failed."
else
