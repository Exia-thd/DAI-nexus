...
PAYLOAD_FILE="$(mktemp "${TMPDIR:-/tmp}/dai-nexus-stop.XXXXXX")" || exit 1
VERIFY_STDERR_FILE="$(mktemp "${TMPDIR:-/tmp}/dai-nexus-verify-stderr.XXXXXX")" || {
  rm -f "$PAYLOAD_FILE"
  exit 1
}
...
safe_reason = ""
if rule_rc:
    safe_reason = "DAI Nexus rule validator rejected the response payload."
patterns = (
...
if not safe_reason and verify_rc:
    safe_reason = "DAI Nexus evidence validator rejected the response payload."
elif not safe_reason:
    safe_reason = "DAI Nexus stop validator rejected the response payload."
...
if isinstance(payload, dict) and platform == "CODEX":
    ...
    if (
        ...
        if safe_id and project_root is not None:
            candidate = project_root / ".dainexus" / "verify" / f"{turn_id}.json"
...
try:
    ...
    payload = {
        "decision": "block",
        "reason": "DAI Nexus verify gate returned invalid protocol output.",
    }
