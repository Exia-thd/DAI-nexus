#!/usr/bin/env bash
# =============================================================================
# dai-nexus-mcp-setup — Universal MCP Setup (Cursor + Claude Code + Antigravity + Codex + Gemini + Zed + OpenCode + Qwen Code)
#
# Single command to set up DAI Nexus MCP for ALL AI clients simultaneously.
...
detect_dai-nexus() {
    ...
    if [[ "$resolved" == */.antigravity/plugins/production-grade/scripts ]]; then
        ...
        if [[ -f "${possible_fw}/AGENTS.md" ]] || [[ -f "${possible_fw}/CLAUDE.md" ]]; then
            ...
            while [[ "$current" != "/" ]] && [[ "$current" != "$HOME" ]]; do
                if [[ -f "${current}/AGENTS.md" ]] || [[ -f "${current}/CLAUDE.md" ]]; then
                    DAINEXUS_DIR="$current"
                    DAINEXUS_IS_PROJECT="true"
                    break
                fi
                [[ -d "${current}/.git" ]] && DAINEXUS_IS_PROJECT="true" && break
                current="$(dirname "$current")"
            done
            [[ "$DAINEXUS_IS_PROJECT" != "true" ]] && DAINEXUS_IS_PROJECT="false"
        fi
    else
        DAINEXUS_DIR="$(dirname "$resolved")"
        DAINEXUS_IS_PROJECT="false"
    fi
}
...
USAGE:
  dai-nexus-mcp-setup.sh [OPTIONS]
...
PLATFORMS:
  ...
  Antigravity   ~/.cursor/projects/<hash>/mcps/user-dai-nexus/
  Codex CLI     ~/.codex/config.toml
  Gemini CLI    ~/.gemini/settings.json
  Zed AI       ~/Library/Application Support/Zed/settings.json (macOS)
               ~/.config/zed/settings.json (Linux)
...
EXAMPLES:
  ...
  # Check status
  dai-nexus-mcp-setup.sh --check

  # Setup Claude Code only
  dai-nexus-mcp-setup.sh --claude-code

  # Setup Claude Desktop only
  dai-nexus-mcp-setup.sh --claude-desktop

  # Setup Antigravity only
  dai-nexus-mcp-setup.sh --antigravity

  # Setup OpenAI Codex CLI only
  dai-nexus-mcp-setup.sh --codex

  # Setup Google Gemini CLI only
  dai-nexus-mcp-setup.sh --gemini

  # Setup Zed AI only
  dai-nexus-mcp-setup.sh --zed

  # Setup OpenCode only
  dai-nexus-mcp-setup.sh --opencode

  # Setup Qwen Code only
  dai-nexus-mcp-setup.sh --qwen
EOF
}
...
check_prerequisites() {
    ...
    if ! command -v node &> /dev/null; then
        log_error "Node.js not found. Install Node.js >= 22.0.0:"
    ...
    if ! node_version_is_supported "$node_version"; then
        log_error "Node.js version too old (found $node_version, need >=22.0.0)"
        exit 1
    fi
...
PY
    ...
    log_ok "DAI Nexus found at $DAINEXUS_DIR"
}
...
detect_platforms() {
    ...
    if [[ -d "$HOME/.cursor/projects" ]]; then
        local ag_count
        ag_count=$(find "$HOME/.cursor/projects" -name "SERVER_METADATA.json" -path "*user-dai-nexus*" 2>/dev/null | wc -l | tr -d ' ')
...
setup_mcp_server() {
    ...
    if [[ -f "${script_dir}/mcp-generate.sh" ]]; then
        if DAINEXUS_DIR_OVERRIDE="$DAINEXUS_DIR" PROJECT_ROOT_OVERRIDE="$PROJECT_ROOT" \
    ...
    # Copy launcher scripts to DAINEXUS_DIR/scripts/
    mkdir -p "${DAINEXUS_DIR}/scripts"
    if cp "${script_dir}/dai-nexus-mcp-launcher.sh" "${DAINEXUS_DIR}/scripts/" 2>/dev/null; then
        chmod +x "${DAINEXUS_DIR}/scripts/dai-nexus-mcp-launcher.sh"
        log_ok "Copied dai-nexus-mcp-launcher.sh"
    fi
}

# ─── Canonical MCP Server ─────────────────────────────────────────────────────
# The canonical MCP server lives at ~/.dainexus/mcp-server/
...
# they reference is always the canonical ~/.dainexus/mcp-server/src/index.ts.

CANONICAL_SERVER_DIR="$HOME/.dainexus/mcp-server"
CANONICAL_SERVER_TS="$CANONICAL_SERVER_DIR/src/index.ts"
CANONICAL_TSX="$CANONICAL_SERVER_DIR/node_modules/.bin/tsx"
INSTALLATION_OWNER_FILE="$HOME/.dainexus/.mcp-server-installation.json"
INSTALLATION_OWNER_MARKER=".dainexus-installation-owner.json"
INSTALLATION_OWNER_VERSION="1"
TRANSACTION_JOURNAL="$HOME/.dainexus/.mcp-setup-transaction.json"
TRANSACTION_DIR=""
TRANSACTION_ACTIVE="false"
TRANSACTION_TOKEN=""
TRANSACTION_MARKER=".dainexus-transaction-owner"
...
def load_owner(path):
    ...
    if value["kind"] != "dai-nexus-mcp-runtime" or value["version"] != int(version):
...
for (const rootKey of rootKeys) {
  for (const managed of ['dai-nexus', 'gitnexus']) {
    ...
    const entry = root.value[rootKey][managed];
    {
      ...
      const ledgerPath = process.env.DAINEXUS_LEDGER_PATH;
      const ledgerTmp = process.env.DAINEXUS_LEDGER_TMP;
      const token = process.env.DAINEXUS_RUNTIME_TOKEN;
      let ledger = { kind: "dai-nexus-mcp-ledger", version: 1, runtime_token: token, records: {} };
      ...
      if (ledgerSource && fs.existsSync(ledgerSource)) {
        try {
          ...
          if (!data || data.kind !== "dai-nexus-mcp-ledger" || data.version !== 1 || data.runtime_token !== token || !data.records) throw new Error('Ledger malformed or token mismatch');
          ledger = data;
        } catch(e) {
...
def inject(point):
    ...
    if os.environ.get("DAINEXUS_TEST_SIGKILL_DURABILITY_AT") == boundary:
...
if os.environ.get("DAINEXUS_TEST_EXTERNAL_WRITE_AT") == label:
    payload = os.environ.get("DAINEXUS_TEST_EXTERNAL_WRITE_CONTENT", "external-writer\n")
...
if os.environ.get("DAINEXUS_TEST_EXTERNAL_WRITE_AT") == label:
    with open(target, "w", encoding="utf-8") as handle:
        handle.write(os.environ.get("DAINEXUS_TEST_EXTERNAL_WRITE_CONTENT", "external-writer\n"))
        handle.flush()
        os.fsync(handle.fileno())
...
if os.environ.get("DAINEXUS_TEST_DURABILITY_FAIL_AT") == f"{label}:before-unlink":
    raise OSError(f"injected durability failure at {label}:before-unlink")
if os.environ.get("DAINEXUS_TEST_SIGKILL_DURABILITY_AT") == f"{label}:before-unlink":
    os.kill(os.getppid(), signal.SIGKILL)
    os._exit(137)
os.unlink(target)
...
config_lock_path() {
    python3 - "$1" "$HOME/.dainexus/locks" <<'PY'
import hashlib
import os
import sys
...
def process_identity(candidate):
    overrides = os.environ.get("DAINEXUS_TEST_PROCESS_IDENTITIES")
    if overrides:
...
PY
    ...
    if [[ -n "${DAINEXUS_TEST_INJECT_SNAPSHOT_RACE_PATH:-}" ]] && \
        [[ -z "${DAINEXUS_TEST_INJECT_SNAPSHOT_RACE_TARGET:-}" || "$target" == "$DAINEXUS_TEST_INJECT_SNAPSHOT_RACE_TARGET" ]]; then
        cat "$DAINEXUS_TEST_INJECT_SNAPSHOT_RACE_PATH" > "$target"
    fi
...
if marker_mode == "transaction":
    ...
    valid = (
        ...
        inside.get("kind") == "dai-nexus-mcp-runtime" and
...
inject = os.environ.get("DAINEXUS_TEST_SIGKILL_DURING_TRASH_CLEANUP") == "1"
injected = False
...
PY
    ...
    case "$action:$had_active" in
        ...
        exchange:true|restoring:true)
            ...
            if [[ "${DAINEXUS_TEST_SIGKILL_AFTER_RECOVERY_RUNTIME:-0}" == "1" ]]; then
                kill -9 "$$"
            fi
            ;;
        move:false)
...
check_atomic_exchange_support() {
    if [[ "${DAINEXUS_TEST_UNSUPPORTED_ATOMIC:-0}" == "1" ]]; then
...
export_runtime_token() {
    if [[ -n "${DAINEXUS_RUNTIME_TOKEN:-}" ]]; then
        return 0
    fi
...
PY
        ...
        export DAINEXUS_RUNTIME_TOKEN="$token"
    else
        # Fresh install
        export DAINEXUS_RUNTIME_TOKEN="$(new_owner_token)"
    fi
    return 0
}

snapshot_ledger() {
    export DAINEXUS_LEDGER_PATH="$HOME/.dainexus/.mcp-config-ledger.json"
    validate_sensitive_path "$DAINEXUS_LEDGER_PATH" file || return 1
    export_runtime_token || return 1

    if [[ "$TRANSACTION_ACTIVE" == "true" ]]; then
        if ! transaction_original_file_state "$DAINEXUS_LEDGER_PATH" >/dev/null 2>&1; then
            transaction_snapshot_file "$DAINEXUS_LEDGER_PATH" || return 1
        fi
    else
        acquire_owned_lock "$(config_lock_path "$DAINEXUS_LEDGER_PATH")" "$DAINEXUS_RUNTIME_TOKEN" || return 1
    fi

    if [[ -f "$DAINEXUS_LEDGER_PATH" ]]; then
        python3 - "$DAINEXUS_LEDGER_PATH" "$DAINEXUS_RUNTIME_TOKEN" <<'PY' || return 1
...
if set(data.keys()) != {"kind", "version", "runtime_token", "records"}:
    ...
    sys.exit(1)
if data.get("kind") != "dai-nexus-mcp-ledger" or data.get("version") != 1:
...
commit_ledger() {
    ...
    if [[ ! -s "$ledger_tmp" ]]; then
        if [[ "$TRANSACTION_ACTIVE" == "true" ]]; then
            if [[ -f "$DAINEXUS_LEDGER_PATH" ]]; then
                transaction_expect_file "$DAINEXUS_LEDGER_PATH" "$DAINEXUS_LEDGER_PATH" || return 1
            else
                transaction_expect_absent "$DAINEXUS_LEDGER_PATH" || return 1
            fi
        fi
        return 0
    fi
    if [[ -f "$DAINEXUS_LEDGER_PATH" ]] && cmp -s "$ledger_tmp" "$DAINEXUS_LEDGER_PATH"; then
        if [[ "$TRANSACTION_ACTIVE" == "true" ]]; then
            transaction_expect_file "$DAINEXUS_LEDGER_PATH" "$ledger_tmp" || return 1
        fi
        return 0
    fi
    local source_state
    if [[ "$TRANSACTION_ACTIVE" == "true" ]]; then
        source_state="$(transaction_current_file_state "$DAINEXUS_LEDGER_PATH")" || return 1
    else
        source_state="$(capture_file_state "$DAINEXUS_LEDGER_PATH")" || return 1
    fi
    chmod 600 "$ledger_tmp"
    if [[ "$TRANSACTION_ACTIVE" == "true" ]]; then
        transaction_expect_file "$DAINEXUS_LEDGER_PATH" "$ledger_tmp" || return 1
    fi
    durable_replace_file "$ledger_tmp" "$DAINEXUS_LEDGER_PATH" "ledger" "$source_state" || return 1
}
...
token = os.environ.get("DAINEXUS_RUNTIME_TOKEN")
ledger = {"kind": "dai-nexus-mcp-ledger", "version": 1, "runtime_token": token, "records": {}}
...
if ledger_source and os.path.exists(ledger_source):
    try:
        with open(ledger_source, encoding="utf-8") as handle:
            data = json.load(handle)
        if (not data or data.get("kind") != "dai-nexus-mcp-ledger" or
...
if ledger_source and os.path.exists(ledger_source):
    try:
        with open(ledger_source, "r") as f:
            data = json.load(f)
            if not data or data.get("kind") != "dai-nexus-mcp-ledger" or data.get("version") != 1 or data.get("runtime_token") != token or "records" not in data:
                raise SystemExit("Ledger malformed or token mismatch")
            ledger = data
...
sync_canonical_server() {
    local src_dir="${DAINEXUS_DIR}/mcp" canonical_parent
    if [[ ! -d "$src_dir" ]]; then
    ...
    trap 'exit 130' INT
    trap 'exit 143' TERM
    if [[ -n "${DAINEXUS_TEST_HOLD_RUNTIME_LOCK_SECONDS:-}" ]]; then
        sleep "$DAINEXUS_TEST_HOLD_RUNTIME_LOCK_SECONDS"
    fi
...
PY
    ...
    if [[ "${DAINEXUS_FORCE_PYTHON_SYNC:-0}" != "1" ]] && command -v rsync >/dev/null 2>&1; then
        rsync -a --exclude='node_modules' --exclude='.dainexus' \
...
preserved = {"node_modules", ".dainexus", ".dainexus-transaction-owner"}
...
PYEOF
    ...
    if [[ -d "$CANONICAL_SERVER_DIR/.dainexus" ]]; then
        cp -a "$CANONICAL_SERVER_DIR/.dainexus" "$CANONICAL_STAGE_DIR/"
    fi
    ...
    local lock_digest marker="$CANONICAL_STAGE_DIR/node_modules/.dainexus-package-lock.sha256"
...
EOF
    ...
    export_runtime_token || return 1
    python3 - "$CANONICAL_STAGE_DIR/$INSTALLATION_OWNER_MARKER" "$DAINEXUS_RUNTIME_TOKEN" \
...
publish_canonical_server() {
    ...
    if [[ "${DAINEXUS_TEST_SIGKILL_AFTER_EXCHANGE:-0}" == "1" ]]; then
        kill -9 "$$"
    fi
...
publish_manifest() {
    ...
    [[ "${DAINEXUS_TEST_FAIL_MANIFEST:-0}" != "1" ]] || return 1
...
setup_antigravity() {
    ...
    # Find existing Antigravity MCP server folder for dai-nexus
    local ag_server_dir=""

    if [[ -d "$HOME/.cursor/projects" ]]; then
        # Look for user-dai-nexus server directory
        while IFS= read -r -d '' dir; do
            if [[ -d "${dir}/tools" ]] && [[ -f "${dir}/SERVER_METADATA.json" ]]; then
                # Check if it's the dai-nexus server
                local meta
                meta=$(cat "${dir}/SERVER_METADATA.json" 2>/dev/null)
                if echo "$meta" | grep -q "dai-nexus"; then
                    ag_server_dir="$dir"
                    break
                fi
            fi
        done < <(find "$HOME/.cursor/projects" -type d -name "user-dai-nexus" -print0 2>/dev/null)
    fi

    if [[ -z "$ag_server_dir" ]]; then
        log_warn "Antigravity dai-nexus server not found in ~/.cursor/projects/"
        log_info "Looking for: ~/.cursor/projects/<hash>/mcps/user-dai-nexus/"
...
#!/usr/bin/env bash
# Antigravity DAI Nexus MCP Launcher
# Uses CANONICAL server at ~/.dainexus/mcp-server/ (never a submodule path)
set -euo pipefail
...
write_dai-nexus_settings() {
    ...
    if [[ -L "$settings_dir" ]] || { [[ -e "$settings_dir" ]] && [[ ! -d "$settings_dir" ]]; }; then
        log_error "DAI Nexus settings directory must be a real directory: $settings_dir"
        return 1
    fi
    ...
    else SHELL_COMPRESSOR="dai-nexus-shell-filter"; fi

    target_tmp="$(mktemp "$settings_dir/.settings.env.XXXXXX")"
    python3 - "$SHELL_COMPRESSOR" "$DAINEXUS_DIR" "$target_tmp" <<'PY' || { rm -f -- "$target_tmp"; return 1; }
import sys
import re
...
with open(target, "w", encoding="utf-8") as f:
    f.write("# DAI Nexus Settings — Generated by dai-nexus-mcp-setup.sh\n")
    f.write(f"export DAINEXUS_SHELL_COMPRESSOR={safe_quote(shell_compressor)}\n")
    f.write(f"export DAINEXUS_SHELL_FILTER_PATH={safe_quote(fw_dir + '/scripts/dai-nexus-shell-filter.sh')}\n")
    f.write("export DAINEXUS_TOKEN_BUDGET='120000'\n")
    f.write("export DAINEXUS_DEDUP_WINDOW='10'\n")
    f.write("export DAINEXUS_SESSION_DEDUP='true'\n")
    f.write("export DAINEXUS_TOOL_SANDBOX='true'\n")
    f.write("export DAINEXUS_MEMORY_ENABLED='true'\n")
...
has_canonical_mcp_config() {
    mcp_config_has_enabled_entry "$1" dai-nexus "$CANONICAL_TSX" "$CANONICAL_SERVER_TS" "${2:-generic}"
}
...
cmd_check() {
    ...
    if [[ "$ANTIGRAVITY_CONFIG" != "not_found" ]] && [[ -d "$ANTIGRAVITY_CONFIG" ]]; then
        ...
        log_warn "  Antigravity dai-nexus server not found"
...
cmd_diagnose() {
    ...
    echo "  PWD:        $(pwd)"
    echo "  DAINEXUS_WORKSPACE: ${DAINEXUS_WORKSPACE:-<not set>}"
    echo "  MCP_WORKSPACE_ROOT:   ${MCP_WORKSPACE_ROOT:-<not set>}"
    echo ""

    log_step "DAI Nexus"
    echo "  DIR:     $DAINEXUS_DIR"
    echo "  PROJECT: $PROJECT_ROOT"
    echo "  EXISTS:  $([ -d "$DAINEXUS_DIR" ] && echo YES || echo NO)"
    echo ""

    log_step "Launchers"
    echo "  dai-nexus: ${DAINEXUS_DIR}/scripts/dai-nexus-mcp-launcher.sh"
    echo "  EXISTS: $([ -f "${DAINEXUS_DIR}/scripts/dai-nexus-mcp-launcher.sh" ] && echo YES || echo NO)"
    echo ""

    log_step "MCP Server"
    local server_dir="${DAINEXUS_DIR}/mcp"
...
prepare_uninstall_candidate() {
    ...
    export_runtime_token || return 1
    export DAINEXUS_LEDGER_PATH="$HOME/.dainexus/.mcp-config-ledger.json"
    if [[ -z "${DAINEXUS_LEDGER_TMP:-}" ]]; then
        export DAINEXUS_LEDGER_TMP="$(mktemp "$TRANSACTION_DIR/ledger.XXXXXX")"
    fi
    case "$kind" in
        jsonc)
...
commit_uninstall_candidates() {
    ...
    if [[ -n "$DAINEXUS_LEDGER_TMP" ]] && [[ -f "$DAINEXUS_LEDGER_TMP" ]]; then
        commit_ledger "$DAINEXUS_LEDGER_TMP" || return 1
        rm -f -- "$DAINEXUS_LEDGER_TMP"
    fi
}
...
cmd_uninstall() {
    ...
    snapshot_ledger || return 1
    if [[ "${DAINEXUS_TEST_UNLINK_LEDGER_AFTER_SNAPSHOT:-0}" == "1" ]]; then
        rm -f -- "$DAINEXUS_LEDGER_PATH"
    fi
    DAINEXUS_LEDGER_TMP=""
    local ledger_state ledger_backup
    ledger_state="$(transaction_original_file_state "$DAINEXUS_LEDGER_PATH")" || return 1
    if [[ "$ledger_state" == present:* ]]; then
        ledger_present="true"
        ledger_backup="$(transaction_original_file_backup "$DAINEXUS_LEDGER_PATH")" || return 1
        DAINEXUS_LEDGER_TMP="$(mktemp "$TRANSACTION_DIR/ledger.XXXXXX")"
        cp "$ledger_backup" "$DAINEXUS_LEDGER_TMP" || return 1
        chmod 600 "$DAINEXUS_LEDGER_TMP"
    fi
    export DAINEXUS_LEDGER_TMP
    ...
    if [[ "$ledger_present" == "true" ]] && ! python3 - \
        "$DAINEXUS_LEDGER_TMP" "$DAINEXUS_RUNTIME_TOKEN" <<'PY'
import json
import os
import sys
...
valid = (
    ...
    ledger.get("kind") == "dai-nexus-mcp-ledger" and
...
PY
    ...
    if [[ "$ledger_present" == "true" ]] && \
        [[ "${DAINEXUS_TEST_UNLINK_LEDGER_AFTER_CANDIDATES:-0}" == "1" ]]; then
        rm -f -- "$DAINEXUS_LEDGER_PATH"
    fi
    if [[ "$ledger_present" == "true" ]]; then
        commit_uninstall_removal "$DAINEXUS_LEDGER_PATH" uninstall-ledger true || return 1
    fi
...
main() {
    ...
    if [[ "$DAINEXUS_IS_PROJECT" == "false" ]] && [[ -f "${DAINEXUS_DIR}/scripts/dai-nexus-submodule-check.sh" ]]; then
        bash "${DAINEXUS_DIR}/scripts/dai-nexus-submodule-check.sh" || true
    fi
    ...
    if [[ -d "$HOME/.cursor/projects" ]]; then
        while IFS= read -r -d '' dir; do
            if [[ -d "${dir}/tools" ]] && grep -q "dai-nexus" "${dir}/SERVER_METADATA.json" 2>/dev/null; then
    ...
    echo ""
    echo -e "${CYAN}⚡ DAI Nexus Universal MCP Setup${NC}"
    echo ""
