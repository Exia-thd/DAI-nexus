...
detect_dai_nexus() {
    ...
    if [[ "$resolved" == */.antigravity/plugins/production-grade/scripts ]]; then
        ...
        # DAINEXUS_ROOT/scripts/mcp/dainexus-mcp-setup.sh
...
EXAMPLES:
  ...
  # Check status
  dainexus-mcp-setup.sh --check

  # Setup Claude Code only
  dainexus-mcp-setup.sh --claude-code

  # Setup Claude Desktop only
  dainexus-mcp-setup.sh --claude-desktop

  # Setup Antigravity only
  dainexus-mcp-setup.sh --antigravity

  # Setup OpenAI Codex CLI only
  dainexus-mcp-setup.sh --codex

  # Setup Google Gemini CLI only
  dainexus-mcp-setup.sh --gemini

  # Setup Zed AI only
  dainexus-mcp-setup.sh --zed

  # Setup OpenCode only
  dainexus-mcp-setup.sh --opencode

  # Setup Qwen Code only
  dainexus-mcp-setup.sh --qwen
EOF
}
...
setup_mcp_server() {
    ...
    if cp "${script_dir}/dainexus-mcp-launcher.sh" "${DAINEXUS_DIR}/scripts/" 2>/dev/null; then
        chmod +x "${DAINEXUS_DIR}/scripts/dainexus-mcp-launcher.sh"
        log_ok "Copied dainexus-mcp-launcher.sh"
    fi
}
...
with open(target, "w", encoding="utf-8") as f:
    f.write("# DAI Nexus Settings — Generated by dainexus-mcp-setup.sh\n")
    f.write(f"export DAINEXUS_SHELL_COMPRESSOR={safe_quote(shell_compressor)}\n")
    f.write(f"export DAINEXUS_SHELL_FILTER_PATH={safe_quote(fw_dir + '/scripts/dainexus-shell-filter.sh')}\n")
...
cmd_diagnose() {
    ...
    log_step "Launchers"
    echo "  dai-nexus: ${DAINEXUS_DIR}/scripts/dainexus-mcp-launcher.sh"
    echo "  EXISTS: $([ -f "${DAINEXUS_DIR}/scripts/dainexus-mcp-launcher.sh" ] && echo YES || echo NO)"
    echo ""
...
main() {
    ...
    if [[ "$DAINEXUS_IS_PROJECT" == "false" ]] && [[ -f "${DAINEXUS_DIR}/scripts/dainexus-submodule-check.sh" ]]; then
        bash "${DAINEXUS_DIR}/scripts/dainexus-submodule-check.sh" || true
    fi
