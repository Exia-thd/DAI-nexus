...
# ── Timeout guard: re-exec self under 30s limit (portable) ────────────────────
if [ -z "$_MCP_VERIFY_TIMEOUT" ]; then
    export _MCP_VERIFY_TIMEOUT=1
    TIMEOUT_CMD=""
    ...
    if [ -n "$TIMEOUT_CMD" ]; then
        exec $TIMEOUT_CMD 30 bash "${BASH_SOURCE[0]}" "$@" || {
            echo "ERROR: verify-mcp-manifest.sh timed out after 30 seconds" >&2
            exit 124
        }
    else
        echo "WARN: timeout/gtimeout not found — running without timeout guard" >&2
    fi
fi
