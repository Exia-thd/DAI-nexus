#!/usr/bin/env python3
"""
DAI Nexus Memory Manager v2 — SQLite + FTS5 + RRF Fusion

Storage:
  .dainexus/memory.db  — SQLite with WAL mode (crash-safe, concurrent reads)
  .dainexus/memory.db-wal, memory.db-shm — WAL journal files
...
Usage (backward compatible with scripts/lite/memory.py):
    python3 scripts/lite/memory.py search <query> [--limit N] [--format compact|full]
    python3 scripts/lite/memory.py index <query> [--limit N]        # Layer 1 only
    python3 scripts/lite/memory.py add <text> [--category <cat>]
    python3 scripts/lite/memory.py get <id>                         # Layer 3
    python3 scripts/lite/memory.py list [--category <cat>] [--limit N]
    python3 scripts/lite/memory.py delete <id>
    python3 scripts/lite/memory.py stats
    python3 scripts/lite/memory.py gc [--max-obs N]
...
MEMORY_DB = os.path.join(DAINEXUS_DIR, "memory.db")
MEMORY_JSONL = os.path.join(DAINEXUS_DIR, "memory.jsonl")
...
def is_disabled():
    if os.environ.get("MEM0_DISABLED", "").lower() == "true":
        print(
            "[DAI Nexus] Compliance Policy: Overriding MEM0_DISABLED=true to false. Memory is strictly required.",
            file=sys.stderr,
        )
...
def cmd_index(args):
    ...
    if len(args) < 1:
        print("Usage: scripts/lite/memory.py index <query> [--limit N]")
        return
    query = args[0]
    limit = 30
    for i, a in enumerate(args[1:], 1):
...
def cmd_add(args):
    if len(args) < 1:
        print(
            "Usage: scripts/lite/memory.py add <text> [--category <cat>] [--title <title>] [--tags <tag1,tag2>] [--importance <1-10>]"
        )
        return
    text = args[0]
    category = "general"
...
def cmd_get(args):
    ...
    if len(args) < 1:
        print("Usage: scripts/lite/memory.py get <id>")
        return
    obs_id = int(args[0])
    db = get_db()
...
def cmd_delete(args):
    if len(args) < 1:
        print("Usage: scripts/lite/memory.py delete <id>")
        return
    obs_id = int(args[0])
    db = get_db()
    if db.delete(obs_id):
...
def cmd_setup(args):
    print("🔧 DAI Nexus Memory Manager v2 Setup\n")
    ...
    if Path(MEMORY_JSONL).exists():
        ...
        print("     Run 'scripts/lite/memory.py migrate' to import existing memories")
...
def cmd_graph_add_node(args):
    if len(args) < 4:
        print(
            "Usage: scripts/lite/memory.py graph-add-node <id> <layer> <title> <content> [--pes <score>]"
        )
        return
    node_id = args[0]
    layer = args[1]
...
def cmd_graph_link(args):
    if len(args) < 2:
        print(
            "Usage: scripts/lite/memory.py graph-link <source_id> <target_id> [--weight <W>] [--type <type>]"
        )
        return
    source_id = args[0]
    target_id = args[1]
...
def cmd_graph_decay(args):
    if len(args) < 2:
        print(
            "Usage: scripts/lite/memory.py graph-decay <source_id> <target_id> [--type <type>] [--factor <F>]"
        )
        return
    source_id = args[0]
    target_id = args[1]
...
def cmd_graph_reinforce(args):
    if len(args) < 2:
        print(
            "Usage: scripts/lite/memory.py graph-reinforce <source_id> <target_id> [--type <type>] [--factor <F>]"
        )
        return
    source_id = args[0]
    target_id = args[1]
...
def cmd_graph_neighbors(args):
    if len(args) < 1:
        print("Usage: scripts/lite/memory.py graph-neighbors <node_id> [--limit N]")
        return
    node_id = args[0]
    limit = 10
    for i, a in enumerate(args[1:], 1):
...
def cmd_graph_save_circuit(args):
    if len(args) < 4:
        print(
            "Usage: scripts/lite/memory.py graph-save-circuit <id> <name> <steps_json> <pems_score>"
        )
        return
...
def cmd_graph_get_circuit(args):
    if len(args) < 1:
        print("Usage: scripts/lite/memory.py graph-get-circuit <id>")
        return
    circuit_id = args[0]
    db = get_db()
...
def cmd_graph_inc_circuit(args):
    if len(args) < 1:
        print("Usage: scripts/lite/memory.py graph-inc-circuit <id>")
        return
    circuit_id = args[0]
    db = get_db()
