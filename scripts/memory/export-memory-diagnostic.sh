#!/usr/bin/env bash
# export-memory-diagnostic.sh — Redacted DAI Nexus memory diagnostics
#
# Usage:
...
export DAINEXUS_SCRIPT_DIR="$SCRIPT_DIR"
...
def collect_memory_bank(stage: Path, workspace: Path, manifest: dict[str, Any]) -> None:
    bank = workspace / ".dainexus" / "memory-bank"
    files = [
...
def collect_config(stage: Path, workspace: Path, manifest: dict[str, Any]) -> None:
    config_files = [
        ...
        (workspace / ".dainexus" / "skills-config.json", "config/skills-config.json"),
        (workspace / ".dainexus" / "mcp-manifest.json", "config/mcp-manifest.json"),
        (workspace / ".antigravity" / "mcp-manifest.json", "config/antigravity-mcp-manifest.json"),
        (workspace / ".dainexus" / "settings.env", "config/settings.env.redacted"),
        (workspace / ".dainexus" / "pipeline-state.json", "config/pipeline-state.json"),
        (workspace / ".dainexus" / "session-track.json", "config/session-track.json"),
    ]
    count = 0
...
def collect_offload(
    ...
    offload = workspace / ".dainexus" / "offload"
...
def collect_audit(stage: Path, workspace: Path, manifest: dict[str, Any]) -> None:
    audit = workspace / ".dainexus" / "audit"
    count = 0
    if audit.exists():
...
def create_archive(stage: Path, output_dir: Path, timestamp: str) -> Path:
    archive = output_dir / f"dai-nexus-memory-diagnostic-{timestamp}.tar.gz"
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Export redacted DAI Nexus memory diagnostics")
...
def main(argv: list[str]) -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="dai-nexus-memory-diagnostic-") as tmp:
        stage = Path(tmp) / "payload"
        stage.mkdir(parents=True, exist_ok=True)
        manifest: dict[str, Any] = {
            "schema": "dai-nexus-memory-diagnostic/v1",
        ...
        db_path = workspace / ".dainexus" / "memory.db"
        write_text(stage, "memory/memory-stats.txt", mem0_stats(workspace))
        ...
        include_redacted_file(
            stage,
            workspace,
            workspace / ".dainexus" / "session-log.json",
            "session/session-log.json",
        )
        include_redacted_file(
            stage,
            workspace,
            workspace / ".dainexus" / "subagent-context" / "CONVERSATION_SUMMARY.md",
