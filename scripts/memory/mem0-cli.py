...
Please use `scripts/lite/memory.py` instead:
  python3 scripts/lite/memory.py setup      # First time setup
  python3 scripts/lite/memory.py add ...   # Add memory
  python3 scripts/lite/memory.py search ... # Search
  python3 scripts/lite/memory.py migrate   # Migrate data first
...
DAI Nexus Memory Manager CLI — persistent project memory, git-versioned.

Storage:
  .dainexus/memory.jsonl  — source of truth, committed to git (compact, human-readable)

Usage:
    python3 scripts/lite/memory.py search <query> [--limit N] [--format compact|full]
    python3 scripts/lite/memory.py add <text> [--category <cat>]
    python3 scripts/lite/memory.py ingest <file1> [file2 ...] [--chunk-size N]
    python3 scripts/lite/memory.py ingest-git [--days N]
    python3 scripts/lite/memory.py summarize <file>
    python3 scripts/lite/memory.py refresh
    python3 scripts/lite/memory.py list [--category <cat>] [--limit N]
    python3 scripts/lite/memory.py delete <memory_id>
    python3 scripts/lite/memory.py export [--format md|json]
    python3 scripts/lite/memory.py stats
    python3 scripts/lite/memory.py gc [--max-memories N]
...
# ── Constants ──
DAINEXUS_DIR = ".dainexus"
MEMORY_LOG = os.path.join(DAINEXUS_DIR, "memory.jsonl")
    ...
    def search(self, query, limit=5):
        ...
        for i, doc_tf in enumerate(self.doc_tfs):
            ...
            for term in query_vec:
                doc_tfidf = doc_tf.get(term, 0) * (
                    math.log((self.n_docs + 1) / (self.df.get(term, 0) + 1)) + 1
                )
...
def extract_structured_facts(text, source_name=""):
    ...
    for line in lines:
        ...
        kv_match = re.match(
            r"^[\-\*]?\s*\*?\*?([A-Za-z\s]+)\*?\*?\s*[:—–]\s*(.+)", line_stripped
        )
        if kv_match:
        ...
        if re.search(
            r"(?i)(decided|chose|selected|using|switched to|migrated to)", line_stripped
        ):
        ...
        if re.search(
            r"(?i)(blocked|waiting|stuck|issue|problem|bug|error|fail)", line_stripped
        ):
            if 20 < len(line_stripped) < 500:
        ...
        if re.search(
            r"(?i)(architecture|stack|framework|database|api|service|endpoint)",
            line_stripped,
        ):
            if 20 < len(line_stripped) < 500:
...
class MemoryStore:
    ...
    def gc(self, max_memories=None):
        ...
        max_m = max_memories or int(
            os.environ.get("MEM0_MAX_MEMORIES", MAX_MEMORIES_DEFAULT)
        )
        entries = self._load()
...
def cmd_search(args):
    ...
    for i, a in enumerate(args[1:], 1):
        if a == "--limit" and i + 1 < len(args):
            limit = int(args[i + 1])  # noqa: E701
        if a == "--format" and i + 1 < len(args):
            fmt = args[i + 1]  # noqa: E701
...
def cmd_add(args):
    if len(args) < 1:
        print("Usage: scripts/lite/memory.py add <text> [--category <cat>]")
        return
    text = args[0]
    category = "general"
    for i, a in enumerate(args[1:], 1):
        if a == "--category" and i + 1 < len(args):
            category = args[i + 1]  # noqa: E701
    store = get_store()
...
def cmd_ingest(args):
    if len(args) < 1:
        print("Usage: scripts/lite/memory.py ingest <file1> [file2 ...]")
        return
    chunk_size = 3000
    files = []
    i = 0
    ...
    for filepath in files:
        ...
        if should_ignore(filepath, ignore_patterns):
            print(f"  ⏭️ {filepath} ignored (.memignore)")
            continue  # noqa: E702
        ...
        for j, chunk in enumerate(chunks):
            src = f"{path.name}" + (
                f" §{j + 1}/{len(chunks)}" if len(chunks) > 1 else ""
            )
...
def cmd_ingest_git(args):
    ...
    if not log:
        print(f"No commits in last {days} days.")
        return  # noqa: E702
...
def cmd_summarize(args):
    ...
    if len(args) < 1:
        print("Usage: scripts/lite/memory.py summarize <file>")
        return  # noqa: E702
    path = Path(args[0])
    if not path.exists():
    ...
    if facts:
        ...
        print(
            f"✅ Summarized {path.name}: {min(len(chunks), 5)} chunk(s) (no structured facts found)"
        )
...
def cmd_refresh(args):
    ...
    for e in old_entries:
        if e.get("source") == "manual" and e.get("category") in (
            "project",
            "architecture",
            "tasks",
        ):
    ...
    if Path("README.md").exists():
        ...
        pipe_match = re.search(
            r"DEFINE\s*→\s*BUILD\s*→\s*HARDEN\s*→\s*SHIP\s*→\s*SUSTAIN", content
        )
        if pipe_match:
            readme_facts.append(
    ...
    store.add(
        f"Project: DAI Nexus v{version} — Production-grade AI pipeline orchestrator",
        category="project",
        source="refresh:identity",
    )
    store.add(
    ...
    try:
        ...
        if log:
            ...
            print("  📋 Git: recent commits ingested")
    except Exception:
        pass

    print(f"\n✅ Memory refreshed for DAI Nexus v{version}")
...
def cmd_delete(args):
    if len(args) < 1:
        print("Usage: scripts/lite/memory.py delete <memory_id>")
        return  # noqa: E702
...
def cmd_stats(args):
    ...
    print(
        f"  Max before GC: {os.environ.get('MEM0_MAX_MEMORIES', MAX_MEMORIES_DEFAULT)}"
    )
...
def cmd_gc(args):
    max_m = None
    for i, a in enumerate(args):
        if a == "--max-memories" and i + 1 < len(args):
            max_m = int(args[i + 1])  # noqa: E701
    store = get_store()
...
def cmd_setup(args):
    print("🔧 DAI Nexus Memory Manager Setup\n")
    os.makedirs(DAINEXUS_DIR, exist_ok=True)
    print(f"  ✅ {DAINEXUS_DIR}/ ready")
