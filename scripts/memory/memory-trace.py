...
def build_parser() -> argparse.ArgumentParser:
    ...
    node = subparsers.add_parser(
        "trace-node", help="Show one offload event and ref preview"
    )
    ...
    session = subparsers.add_parser(
        "trace-session", help="List offload events for a session"
    )
    ...
    canvas = subparsers.add_parser(
        "trace-canvas", help="Print Mermaid canvas for a session"
    )
