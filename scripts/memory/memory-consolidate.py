#!/usr/bin/env python3
"""
Consolidate DAI Nexus atom/offload memory into scenario and persona layers.

Outputs:
  .dainexus/memory-bank/persona.md
  .dainexus/memory-bank/scenarios/<scenario_id>.md
"""
...
DEFAULT_MEMORY_DB = ".dainexus/memory.db"
DEFAULT_MEMORY_BANK = ".dainexus/memory-bank"
DEFAULT_OFFLOAD_DIR = ".dainexus/offload"
DEFAULT_SESSION_LOG = ".dainexus/session-log.json"

PREFERENCE_PATTERNS = re.compile(
    r"\b(always|avoid|decision|decided|default|dai-nexus|gate|gitnexus|memory|must|never|prefer|rule|use)\b",
    re.IGNORECASE,
)
...
def source_refs(
    ...
    for obs in observations:
        refs.append(f"memory:{obs.get('id')}")
    filtered_events = [
        event
        for event in events
    ...
    deduped: list[str] = []
    for ref in refs:
        if ref not in deduped and not ref.endswith("/None") and ref != "memory:None":
            deduped.append(ref)
...
def preference_bullets(
    ...
    for obs in observations:
        ...
        if not PREFERENCE_PATTERNS.search(text):
            continue
        bullets.append(
            f"- {one_line(obs.get('content') or obs.get('title'), 220)} [memory:{obs.get('id')}]"
        )
...
def render_persona(
    ...
    project_defaults = [
        "- Use DAI Nexus pipeline gates for staged implementation when an active pipeline exists.",
