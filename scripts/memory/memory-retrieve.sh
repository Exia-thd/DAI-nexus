#!/usr/bin/env bash
# memory-retrieve.sh — Adaptive Memory Retrieval for DAI Nexus
...
PERSONA_FILE="$DAINEXUS_DIR/.dainexus/memory-bank/persona.md"
SCENARIOS_DIR="$DAINEXUS_DIR/.dainexus/memory-bank/scenarios"
...
check_deps() {
    if [[ ! -f "$MEM0_SCRIPT" ]]; then
        error "scripts/lite/memory.py not found at $MEM0_SCRIPT"
    fi
...
# Run memory search and capture output
run_mem0_search() {
    local query="$1"
    local limit="$2"
    if [[ -z "$query" ]]; then
...
# Run memory index and capture output
run_mem0_index() {
    local query="$1"
    local limit="$2"
    if [[ -z "$query" ]]; then
...
try:
    ...
    pass
" 2>/dev/null || true)
    emit_limited_section "Relevant Memories (memory)" "$formatted" "$max_tokens"
}
...
main() {
    ...
    echo ""
    echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
    echo "  MEMORY RETRIEVAL — DAI Nexus Step 0.5"
    echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
    echo ""
    ...
    # Run memory search (Layer 2 - FTS + BM25)
    ...
    if [[ "$actual_search_limit" -gt 0 ]]; then
        ...
        search_results="[]"
        info "Skipping memory atom search — context budget already used by higher layers"
    fi

    # Run memory index (Layer 1 - compact)
    local index_results
