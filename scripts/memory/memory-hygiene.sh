...
# Runs garbage collection, deduplication, and archival on memory
...
if [[ "$DRY_RUN" == "false" ]]; then
    ...
    echo "[DRY RUN] Would run: scripts/lite/memory.py gc --max-obs $MAX_OBS"
fi
...
if old_count > 20:
    print(f'Old sessions: {old_count} (consider running: scripts/lite/memory.py gc --category session --older-than 30d)')
else:
