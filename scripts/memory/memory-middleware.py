...
MEMORY_DB_DIR = Path(os.environ.get("MEMORY_DB_DIR", f"{HOME}/.dainexus/sessions"))
...
# Resolution: DAINEXUS_SESSION_LOG env > ~/.dainexus/session-log.json > .dainexus/session-log.json
...
def get_session_log_path() -> Path:
    ...
    Resolution order:
      ...
      2. ~/.dainexus/session-log.json (home-relative, if exists)
      3. .dainexus/session-log.json (repo-relative, DEFAULT)
    """
    if DAINEXUS_SESSION_LOG:
        return Path(DAINEXUS_SESSION_LOG)

    home_path = HOME / ".dainexus" / "session-log.json"
    repo_path = WORKSPACE_ROOT / ".dainexus" / "session-log.json"
...
def auto_ingest_session_decisions(session: dict) -> int:
    """
    Auto-ingest key decisions from completed session into memory.
...
def save_to_mem0(summary: str, checkpoint_id: str) -> bool:
    ...
    try:
        ...
        warn("memory save timed out, continuing without memory update")
        return False
    except Exception as e:
        warn(f"Could not save to memory: {e}")
        return False
