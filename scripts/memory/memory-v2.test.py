...
spec = importlib.util.spec_from_file_location("memory_v2", scripts_dir / "memory-v2.py")
...
def run_tests():
    ...
    try:
        ...
        test("memory_index returns layer 1", lambda: _raise(e))  # noqa: F821
    finally:
        cleanup_db(db)
    ...
    try:
        ...
        test("memory_search returns layer 2", lambda: _raise(e))  # noqa: F821
    finally:
        cleanup_db(db)
    ...
    try:
        ...
        test("memory_search increments access count", lambda: _raise(e))  # noqa: F821
    finally:
        cleanup_db(db)
    ...
    try:
        ...
        test("memory_get returns full observation", lambda: _raise(e))  # noqa: F821
    finally:
        cleanup_db(db)
    ...
    try:
        ...
        test("memory_get returns None for missing", lambda: _raise(e))  # noqa: F821
    finally:
        cleanup_db(db)
    ...
    try:
        ...
        test("add_observation creates entry", lambda: _raise(e))  # noqa: F821
    finally:
        cleanup_db(db)
    ...
    try:
        ...
        test("add detects duplicates", lambda: _raise(e))  # noqa: F821
    finally:
        cleanup_db(db)
    ...
    try:
        ...
        test("add with tags", lambda: _raise(e))  # noqa: F821
    finally:
        cleanup_db(db)
    ...
    try:
        ...
        test("list_all returns all observations", lambda: _raise(e))  # noqa: F821
    finally:
        cleanup_db(db)
    ...
    try:
        ...
        test("list_all with category filter", lambda: _raise(e))  # noqa: F821
    finally:
        cleanup_db(db)
    ...
    try:
        ...
        conn = db.get_connection()
        cursor = conn.execute(
            "SELECT archived FROM observations WHERE id = ?", (entry["id"],)
        )
        row = cursor.fetchone()
        assert row["archived"] == 1
        test("delete archives (soft delete)", lambda: None)
    except Exception:
        test("delete archives (soft delete)", lambda: _raise(e))  # noqa: F821
    finally:
        cleanup_db(db)
    ...
    try:
        ...
        test("count active observations", lambda: _raise(e))  # noqa: F821
    finally:
        cleanup_db(db)
    ...
    try:
        ...
        test("stats returns correct data", lambda: _raise(e))  # noqa: F821
    finally:
        cleanup_db(db)
    ...
    try:
        ...
        test("gc archives low-value observations", lambda: _raise(e))  # noqa: F821
    finally:
        cleanup_db(db)
    ...
    try:
        ...
        test("gc preserves pinned", lambda: _raise(e))  # noqa: F821
    finally:
        cleanup_db(db)
    ...
    try:
        ...
        test("gc does nothing when under limit", lambda: _raise(e))  # noqa: F821
    finally:
        cleanup_db(db)
    ...
    try:
        ...
        test("rrf_merge ranks by reciprocal rank", lambda: _raise(e))  # noqa: F821
    finally:
        cleanup_db(db)
    ...
    try:
        ...
        test("rrf_merge handles empty lists", lambda: _raise(e))  # noqa: F821
    finally:
        cleanup_db(db)
    ...
    try:
        ...
        test("rrf_merge handles missing IDs", lambda: _raise(e))  # noqa: F821
    finally:
        cleanup_db(db)
    ...
    try:
        ...
        test("migrate from JSONL", lambda: _raise(e))  # noqa: F821
    finally:
        cleanup_db(db)
    ...
    try:
        ...
        test("migrate handles missing file", lambda: _raise(e))  # noqa: F821
    finally:
        cleanup_db(db)
    ...
    try:
        ...
        test("search returns backward-compatible format", lambda: _raise(e))  # noqa: F821
    finally:
        cleanup_db(db)
    ...
    try:
        # 1. Test add_node
        n1_ok = db.add_node(
            "node-1", "semantic", "Memory Schema", "Content about memory schema"
        )
        n2_ok = db.add_node(
            "node-2",
            "episodic",
            "Schema Migration Run",
            "Trace of schema migration execution",
            pes_score=0.0,
        )
        n3_ok = db.add_node(
            "node-3",
        ...
        reinforce_ok = db.reinforce_edge(
            "node-2", "node-3", edge_type="solves", factor=1.2
        )
        assert reinforce_ok
        ...
        test("relational graph functionalities", lambda: _raise(e))  # noqa: F821
    finally:
        cleanup_db(db)
