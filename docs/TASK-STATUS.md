...
|---------|------|--------|-------|
| **P1-T1** | I-NEW-1.1: Shell Filter native | `31a8dd8` | `scripts/dai-nexus-shell-filter.sh` — 384 lines, 18 command filters. ANSI stripping, structured summaries, per-file diff parsing. macOS awk compatible. Synced to Antigravity plugin. |
...
| **P1-T3** | I-NEW-1.3: RTK Detection in MCP setup | `31a8dd8` | `scripts/dai-nexus-mcp-setup.sh` — Detects rtk, chop, snip, ctx, tkill at setup time. Writes `.dainexus/settings.env` with `DAINEXUS_SHELL_COMPRESSOR`. |
...
### P2 — DAI Nexus Node Code Intelligence (Medium Priority)

| Task ID | Name | Commit | Notes |
|---------|------|--------|-------|
| **P2-T1** | I5: DAI Nexus Node Outline Mode tool | `c6ab98c` | `dainexus-node/src/mcp/outline.ts` + `outline.test.ts`. Pattern-based structural extraction for TS/JS/Python/Go/Rust/Java/C++. Thresholds: >200 lines or >6000 tokens triggers outline mode. 19 unit tests. |
| **P2-T2** | I-NEW-2: DAI Nexus Node Callee Footer + Session Dedup | `HEAD` | `dainexus-node/src/mcp/outline.ts` + `tools.ts`. Extended DedupState to track contextUids separately. Added `checkContextDedup()` function. Context tool shows callee footer (top 5 call targets inline). 24 passing tests. |

### P3 — Memory Engine (High Priority)

| Task ID | Name | Commit | Notes |
|---------|------|--------|-------|
| **P3-T1** | I-NEW-3: Memory Engine v2 (SQLite + FTS5 + RRF) | `HEAD` | `scripts/lite/memory.py` — SQLite + FTS5 thay thế JSONL + TF-IDF. WAL mode, FTS5 BM25 ranking. 3-layer progressive disclosure: Layer 1 (~15 tokens), Layer 2 (~60 tokens), Layer 3 (~200 tokens). RRF Fusion cho hybrid search. 30 passing tests. |
...
| **P5-T2** | I-NEW-4: Context-Mode Integration (ctx_execute MCP tool) | `HEAD` | `dainexus-node/src/mcp/tools.ts` — New `ctx_execute` tool. Sandboxed code execution with structured output summarization. Supports: python, node, bash, go, rust, ruby, php. 173 passing tests. |
| **P5-T3** | I-NEW-6: Token-Savior Integration | `HEAD` | `scripts/dai-nexus-mcp-setup.sh` — Token-Savior detection. Sets `DAINEXUS_CODE_NAV` (token-savior > dainexus-node) và `DAINEXUS_MEMORY_VECTOR` (token-savior > sqlite). Synced to Antigravity plugin. |
...
| File | Purpose |
|------|---------|
| `scripts/dai-nexus-shell-filter.sh` | Shell output filter (main) |
| `scripts/dai-nexus-mcp-setup.sh` | MCP setup with RTK + Token-Savior detection |
...
| `dainexus-node/src/mcp/outline.ts` | Outline mode + session dedup |
| `dainexus-node/src/mcp/tools.ts` | All MCP tools including ctx_execute |
| `dainexus-node/src/mcp/ctx-execute.test.ts` | 22 unit tests for ctx_execute |
| `scripts/lite/memory.py` | Memory v2: SQLite + FTS5 + RRF |
| `scripts/memory-v2.test.py` | 30 unit tests for Memory v2 |
...
| `dainexus-node/src/` | DAI Nexus Node code intelligence engine |
...
3. Run `cd dainexus-node && npm test` to verify baseline
