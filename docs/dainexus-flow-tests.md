# DAI Nexus Flow Test Plan

> **Mục tiêu:** Verify toàn bộ DAI Nexus flows hoạt động đúng — từ user request → classification → routing → skill execution → quality gate.
>
...
|-------|-----------|-----------|--------|
| **L0 — Infrastructure** | Auto-init, DAI Nexus Node index | 3 | ⬜ |
...
### L0-T1: Auto-Initialization — DAI Nexus Node Missing
...
### L0-T2: Auto-Initialization — DAI Nexus Node Exists

```
Setup:   .dainexus/mcp-server/mcp-config.json tồn tại
Request:  "Build a React app"
...
Expect:   Nếu có update: thông báo "DAI Nexus update available (vX.X.X → vY.Y.Y). Run /update to upgrade."
         Nếu không: im lặng
Status:   ⬜
```
...
```
Setup:   .dainexus/settings.md = "Execution: parallel"
...
```
Setup:   .dainexus/codebase-context.md tồn tại
...
|------|---------|--------|---------|
| L0-T1 | DAI Nexus Node MCP config | ✅ PASS | `.dainexus/mcp-server/mcp-config.json` exists |
...
| `.dainexus/codebase-context.md` missing | INFO | Optional — greenfield mode | Create when onboarding existing project |
