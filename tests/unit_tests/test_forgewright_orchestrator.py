...
SPEC = importlib.util.spec_from_file_location("dai-nexus_orchestrator", MODULE_PATH)
...
def test_model_precedence_and_empty_fallback() -> None:
    assert (
        orchestrator.resolve_model(
            {"DAINEXUS_MODEL": " gpt-primary ", "NINEROUTER_MODEL": "legacy"}
        )
        == "gpt-primary"
    )
    assert (
        orchestrator.resolve_model(
            {"DAINEXUS_MODEL": " ", "NINEROUTER_MODEL": "legacy"}
        )
        == "legacy"
    )
    with pytest.raises(ValueError, match="DAINEXUS_MODEL is required"):
        orchestrator.resolve_model({})
...
def test_api_key_precedence_and_minimax_key_is_not_accepted() -> None:
    assert (
        orchestrator.resolve_api_key(
            {"DAINEXUS_API_KEY": " configured ", "NINEROUTER_API_KEY": "legacy"}
        )
        == "configured"
    )
...
def test_api_url_is_preserved_exactly(url: str) -> None:
    assert orchestrator.resolve_api_url({"DAINEXUS_API_URL": url}) == url
...
def test_api_url_requires_explicit_configuration() -> None:
    with pytest.raises(
        ValueError,
        match="DAINEXUS_API_URL or NINEROUTER_BASE_URL must be explicitly configured",
    ):
        orchestrator.resolve_api_url({})
...
def test_runtime_limits_have_no_default_turn_quota() -> None:
    ...
    assert (
        orchestrator.RuntimeLimits.from_env({"DAINEXUS_MAX_TURNS": "0"}).max_turns
        is None
    )


def test_runtime_limits_reject_invalid_turn_caps() -> None:
    with pytest.raises(ValueError, match="DAINEXUS_MAX_TURNS"):
        orchestrator.RuntimeLimits.from_env({"DAINEXUS_MAX_TURNS": "many"})
    with pytest.raises(ValueError, match="DAINEXUS_MAX_TURNS"):
        orchestrator.RuntimeLimits.from_env({"DAINEXUS_MAX_TURNS": "-1"})
...
def test_runtime_source_wires_fail_closed_limits_and_workspace() -> None:
    ...
    assert 'DAINEXUS_TOOL_SANDBOX"] = "false"' not in source
