...
def create_memory_db(path: Path) -> None:
    ...
    conn.execute(
        ...
        (
            ...
            "decisions",
            "Use DAI Nexus gates",
            "Decision: always use DAI Nexus gate approval before moving phases.",
            "manual",
            "2026-06-23T00:00:00",
            1,
            0,
...
def create_offload_events(workspace: Path) -> None:
    session_dir = workspace / ".dainexus" / "offload" / "session-a"
...
def test_consolidate_writes_persona_and_scenario_with_source_refs(tmp_path):
    ...
    create_session_log(workspace / ".dainexus" / "session-log.json")
    ...
    assert "schema: dai-nexus-memory-persona/v1" in persona.read_text()
    assert "memory:1" in persona.read_text()
    assert "offload:session-a/n1" in persona.read_text()
    assert "schema: dai-nexus-memory-scenario/v1" in scenario.read_text()
