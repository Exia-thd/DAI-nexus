...
if [[ ! -x "$DAINEXUS_DIR/scripts/lite/stop-gate.sh" ]]; then
    echo "FAILED: Stop gate runtime was not installed"
    exit 1
fi
if [[ ! -x "$DAINEXUS_DIR/scripts/lite/gemini-before-tool-gate.sh" ]]; then
    echo "FAILED: Gemini BeforeTool gate runtime was not installed"
    exit 1
fi
if [[ ! -x "$DAINEXUS_DIR/scripts/lite/antigravity-pre-tool-gate.sh" ]]; then
    echo "FAILED: Antigravity PreToolUse gate runtime was not installed"
    exit 1
fi
if [[ ! -x "$DAINEXUS_DIR/scripts/lite/run-check.sh" ]] || [[ ! -x "$DAINEXUS_DIR/scripts/lite/run_check.py" ]]; then
...
gemini_before_schema=$(node -e "var c=JSON.parse(require('fs').readFileSync('$HOME/.gemini/settings.json')); console.log(Array.isArray(c.hooks.BeforeTool) && c.hooks.BeforeTool.some(g => g.matcher === '*' && Array.isArray(g.hooks) && g.hooks.some(h => h.name === 'dai-nexus-policy' && h.type === 'command' && h.command.includes('gemini-before-tool-gate.sh') && typeof h.timeout === 'number')));")
if [[ "$gemini_before_schema" != "true" ]]; then
    echo "FAILED: Gemini BeforeTool policy hook schema is invalid"
    exit 1
fi
antigravity_schema=$(node -e "var c=JSON.parse(require('fs').readFileSync('$HOME/.gemini/config/hooks.json')); var n=c['dai-nexus-policy']; console.log(Boolean(n && Array.isArray(n.PreToolUse) && n.PreToolUse.some(g => g.matcher === '*' && Array.isArray(g.hooks) && g.hooks.some(h => h.type === 'command' && h.command.includes('antigravity-pre-tool-gate.sh') && Number.isInteger(h.timeout) && h.timeout > 0))));")
...
gemini_repaired=$(node -e "var c=JSON.parse(require('fs').readFileSync('$HOME/.gemini/settings.json')); console.log(Array.isArray(c.hooks.BeforeTool) && c.hooks.BeforeTool.some(g => g.matcher === '*' && Array.isArray(g.hooks) && g.hooks.some(h => h.name === 'dai-nexus-policy' && h.type === 'command' && h.command.includes('gemini-before-tool-gate.sh') && typeof h.timeout === 'number')));")
antigravity_repaired=$(node -e "var c=JSON.parse(require('fs').readFileSync('$HOME/.gemini/config/hooks.json')); var n=c['dai-nexus-policy']; console.log(Boolean(n && Array.isArray(n.PreToolUse) && n.PreToolUse.some(g => g.matcher === '*' && Array.isArray(g.hooks) && g.hooks.some(h => h.command.includes('antigravity-pre-tool-gate.sh') && Number.isInteger(h.timeout) && h.timeout > 0))));")
