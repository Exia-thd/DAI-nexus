...
# ─── Config ─────────────────────────────────────────────────────
DAINEXUS_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd -P)"
SCRIPT_DIR="$DAINEXUS_DIR/scripts/mcp"
...
seed_canonical_dependencies() {
    local target_server="$TEST_HOME/.dainexus/mcp-server"
    mkdir -p "$target_server/src"
    cp -a "$DAINEXUS_DIR/mcp/node_modules" "$target_server/node_modules" || true
    cp "$DAINEXUS_DIR/mcp/package-lock.json" "$target_server/" || true
    ...
    cat > "$target_server/.dainexus-installation-owner.json" <<EOF
{"kind": "dai-nexus-mcp-runtime", "version": 1, "token": "12345-00000000000000000000000000000000", "path": "$target_server", "lockfile_sha256": "$sha256"}
EOF
    mkdir -p "$TEST_HOME/.dainexus"
    cp "$target_server/.dainexus-installation-owner.json" \
        "$TEST_HOME/.dainexus/.mcp-server-installation.json"
}

mark_owned_test_runtime() {
    local home="$1" runtime="$1/.dainexus/mcp-server" digest token
    mkdir -p "$runtime/src" "$runtime/node_modules/.bin" "$runtime/node_modules/tsx/dist"
    cp "$DAINEXUS_DIR/mcp/package-lock.json" "$runtime/package-lock.json"
    ...
    python3 - "$runtime/.dainexus-installation-owner.json" \
        "$home/.dainexus/.mcp-server-installation.json" "$runtime" "$digest" "$token" <<'PY'
import json
import os
import sys
internal, external, runtime, digest, token = sys.argv[1:]
owner = {"kind": "dai-nexus-mcp-runtime", "version": 1, "token": token,
...
$output"
    ...
    [[ -x "${TEST_HOME}/.dainexus/mcp-server/node_modules/.bin/tsx" ]] && pass "Canonical tsx executable" || fail "Canonical tsx executable missing"
    [[ -s "${TEST_HOME}/.dainexus/mcp-server/node_modules/.dainexus-package-lock.sha256" ]] && pass "Dependency lock digest recorded" || fail "Dependency lock digest missing"
    if node - "$DAINEXUS_DIR/mcp/package.json" "$DAINEXUS_DIR/mcp/package-lock.json" <<'NODE'
...
NODE
    ...
    [[ -f "${TEST_HOME}/.dainexus/mcp-server/build/runtime/tool-execution-gateway.js" ]] && pass "Gateway build artifact" || fail "Gateway build artifact missing"
    ...
    grep -Fq "${TEST_HOME}/.dainexus/mcp-server/node_modules/.bin/tsx" "${TEST_HOME}/.cursor/mcp.json" && pass "Cursor uses canonical tsx" || fail "Cursor does not use canonical tsx"
    grep -Fq "${TEST_HOME}/.dainexus/mcp-server/src/index.ts" "${TEST_HOME}/.cursor/mcp.json" && pass "Cursor uses canonical server source" || fail "Cursor does not use canonical server source"
    ...
    cp -a "${TEST_HOME}/.dainexus/mcp-server" "$runtime_before"
    ...
    if HOME="$TEST_HOME" PATH="$fake_npm_bin:$PATH" \
        ...
        [[ ! -e "${TEST_HOME}/.dainexus/.mcp-server.setup.lock" ]] && \
        ! find "${TEST_HOME}/.dainexus" -maxdepth 1 -name '.mcp-server.stage.*' -print -quit | grep -q .; then
...
NODE
    ...
    cp -a "${TEST_HOME}/.dainexus/mcp-server" "${TEST_PROJECT}/runtime-before-manifest-failure"
    ...
    if HOME="$TEST_HOME" DAINEXUS_TEST_FAIL_MANIFEST=1 \
        ...
        cmp -s "${TEST_PROJECT}/settings-before-publish-failure.env" \
            "${TEST_PROJECT}/.dainexus/settings.env"; then
...
NODE
    touch "${TEST_HOME}/.dainexus/mcp-server/runtime-before-crash.marker"
    cp -a "${TEST_HOME}/.dainexus/mcp-server" "${TEST_PROJECT}/runtime-before-crash"
    ...
    if HOME="$TEST_HOME" DAINEXUS_TEST_SIGKILL_AFTER_RECOVERY_RUNTIME=1 \
        ...
        ! find "${TEST_HOME}/.dainexus" -maxdepth 1 -name '.mcp-server.stage.*' -print -quit | grep -q . && \
    ...
    recovery_token="$(node -p "JSON.parse(require('fs').readFileSync(process.argv[1], 'utf8')).token" \
        "${TEST_HOME}/.dainexus/.mcp-setup-transaction.json")"
    recovery_trash="${TEST_HOME}/.dainexus/.mcp-trash/runtime-${recovery_token}"
    foreign_trash="${TEST_HOME}/.dainexus/.mcp-trash/runtime-999-abcdef"
    ...
    if HOME="$TEST_HOME" bash "${SCRIPT_DIR}/dainexus-mcp-setup.sh" --check  && \
        ...
        [[ ! -e "${TEST_HOME}/.dainexus/.mcp-setup-transaction.json" ]] && \
        [[ ! -e "$recovery_trash" ]] && [[ -f "$foreign_target/sentinel" ]] && \
        [[ -L "$foreign_trash/runtime" ]] && \
        ! find "${TEST_HOME}/.dainexus" -maxdepth 1 -name '.mcp-server.stage.*' -print -quit | grep -q . && \
        ! find "${TEST_HOME}/.dainexus/.transactions" -mindepth 1 -print -quit 2>/dev/null | grep -q . && \
        ! find "${TEST_HOME}/.dainexus" -type d -name '*.lock' -print -quit | grep -q .; then
...
NODE
    ...
    mkdir -p "${TEST_HOME}/.dainexus/mcp-server/src/stale"
...
NODE
    ...
    if wait "$concurrent_one" && wait "$concurrent_two" && \
        ...
        [[ ! -e "${TEST_HOME}/.dainexus/.mcp-server.setup.lock" ]]; then
...
NODE
    ...
    local identity_lock="$TEST_HOME/.dainexus/identity.lock"
    ...
    if HOME="$TEST_HOME" DAINEXUS_TEST_PROCESS_IDENTITIES="$identity_overrides" bash -c '
        source "$1"
        token="$(new_owner_token)"
        acquire_owned_lock "$2" "$token"
        release_owned_lock "$2" "$token"
...
test_settings_and_durability_transactions() {
    ...
    setup_test_project
    mkdir -p "$TEST_HOME" "$TEST_PROJECT/.dainexus"
    local settings="$TEST_PROJECT/.dainexus/settings.env"
    ...
    rm -rf "$TEST_HOME/.dainexus" "$settings"
    ...
    if HOME="$TEST_HOME" DAINEXUS_TEST_DURABILITY_FAIL_AT="settings-env:after-replace" \
        bash -c '
            source "$1"
            PROJECT_ROOT="$2"
            DAINEXUS_DIR="$3"
    ...
    if HOME="$TEST_HOME" DAINEXUS_TEST_SIGKILL_DURABILITY_AT="settings-env:after-replace" \
        bash -c '
            source "$1"
            PROJECT_ROOT="$2"
            DAINEXUS_DIR="$3"
...
with open(path, "rb") as handle:
    fw = tomllib.load(handle)["mcp_servers"]["dai-nexus"]
raise SystemExit(0 if fw["command"] == f"{runtime}/node_modules/.bin/tsx" and
...
PY
    ...
    if ! grep -Fq 'dai-nexus: CONFIGURED' <<< "$check_output" && \
        ...
        grep -Fq 'dai-nexus: NOT configured with canonical server' <<< "$diagnose_output"; then
...
test_all_isolated() {
    ...
    local zed_dir desktop_dir antigravity_dir="$TEST_PROJECT/all-home/.cursor/projects/hash/mcps/user-dai-nexus"
    ...
    cat > "$desktop_dir/claude_desktop_config.json" <<EOF
{
  ...
  "mcpServers": {
    "dai-nexus": {
      "command": "$all_home/.dainexus/mcp-server/node_modules/.bin/tsx",
      "args": ["$all_home/.dainexus/mcp-server/src/index.ts"],
    },
...
test_uninstall_contracts() {
    ...
    local generic_fixture='{"keep":"unchanged","mcpServers":{"dai-nexus":{"command":"'$uninstall_home'/.dainexus/mcp-server/node_modules/.bin/tsx"},"gitnexus":{"command":"'$uninstall_home'/.dainexus/gitnexus"},"other":{"command":"keep"}}}'
    ...
    cat > "$desktop_config" <<'EOF'
{
  ...
  "desktopKeep": true,
  "mcpServers": {
    "dai-nexus": { "command": "stale", },
    "gitnexus": { "command": "stale", },
    "other": { "command": "keep", "args": [], },
  },
}
EOF
    printf '{"keep":"zed","context_servers":{"dai-nexus":{"command":"'$uninstall_home'/.dainexus/mcp-server/node_modules/.bin/tsx"},"gitnexus":{"command":"'$uninstall_home'/.dainexus/gitnexus"},"other":{"command":"keep"}}}\n' \
        > "$zed_config"
    ...
    cat > "$opencode_root/opencode.jsonc" <<'EOF'
{
  ...
  "keep": "opencode",
  "mcp": {
    "dai-nexus": { "type": "local", "command": ["'$uninstall_home'/.dainexus/mcp-server/node_modules/.bin/tsx"], },
    "gitnexus": { "type": "local", "command": ["'$uninstall_home'/.dainexus/gitnexus"], },
...
EOF
    ...
    mkdir -p "$uninstall_home/.dainexus/mcp-server/large-owned-payload/chunks"
    for index in {1..64}; do
        printf 'owned-runtime-%s\n' "$index" \
            > "$uninstall_home/.dainexus/mcp-server/large-owned-payload/chunks/$index"
    done
...
fs.writeFileSync(ledgerPath, JSON.stringify({kind: 'dai-nexus-mcp-ledger', version: 1, runtime_token: token, records}, null, 2));
NODE
...
PY
    ...
    if HOME="$uninstall_home" XDG_CONFIG_HOME="$uninstall_xdg" \
        DAINEXUS_TEST_SIGKILL_DURING_TRASH_CLEANUP=1 \
        bash "$fresh_script" --uninstall ; then
        fail "Runtime uninstall cleanup SIGKILL unexpectedly succeeded"
    elif [[ ! -e "$uninstall_home/.dainexus/mcp-server" ]] && \
        find "$uninstall_home/.dainexus/.mcp-trash" -name owner.json -print -quit | grep -q .; then
...
PY
    then
        if grep -Fq '# Preserve Codex unrelated comment.' "$uninstall_home/.codex/config.toml" && \
            ...
            [[ ! -e "$uninstall_home/.dainexus/mcp-server" ]] && \
    ...
    mkdir -p "$uninstall_home/.dainexus/mcp-server/from-home"
    printf 'remove-from-home\n' > "$uninstall_home/.dainexus/mcp-server/from-home/sentinel"
    mark_owned_test_runtime "$uninstall_home"
    if (cd "$uninstall_home" && HOME="$uninstall_home" XDG_CONFIG_HOME="$uninstall_xdg" \
        bash "$fresh_script" --uninstall ) && \
        [[ ! -e "$uninstall_home/.dainexus/mcp-server" ]]; then
    ...
    mkdir -p "$uninstall_home/.dainexus/mcp-server"
    printf 'runtime-must-stay\n' > "$uninstall_home/.dainexus/mcp-server/runtime-sentinel"
    mark_owned_test_runtime "$uninstall_home"
    local output
    if output="$(HOME="$uninstall_home" XDG_CONFIG_HOME="$uninstall_xdg" \
        ...
        [[ "$(cat "$uninstall_home/.dainexus/mcp-server/runtime-sentinel")" == "runtime-must-stay" ]] && \
        [[ -f "$uninstall_home/.dainexus/.mcp-server-installation.json" ]]; then
    ...
    mkdir -p "$race_home/.cursor" "$race_home/.dainexus/mcp-server"
    printf '%s\n' "$generic_fixture" > "$race_home/.cursor/mcp.json"
    printf 'runtime-must-survive-conflict\n' > "$race_home/.dainexus/mcp-server/sentinel"
    mark_owned_test_runtime "$race_home"
    if HOME="$race_home" XDG_CONFIG_HOME="$TEST_PROJECT/race-xdg" \
        DAINEXUS_TEST_EXTERNAL_WRITE_AT=uninstall-jsonc \
        DAINEXUS_TEST_EXTERNAL_WRITE_CONTENT='{"external":"wins"}' \
        bash "$fresh_script" --uninstall ; then
        fail "Uninstall overwrote a non-cooperative client writer"
    elif [[ "$(cat "$race_home/.cursor/mcp.json")" == '{"external":"wins"}' ]] && \
        [[ "$(cat "$race_home/.dainexus/mcp-server/sentinel")" == "runtime-must-survive-conflict" ]] && \
        [[ -f "$race_home/.dainexus/.mcp-server-installation.json" ]]; then
    ...
    mkdir -p "$foreign_home/.dainexus/mcp-server" "$foreign_home/.cursor"
    printf 'foreign-runtime\n' > "$foreign_home/.dainexus/mcp-server/sentinel"
    ...
    elif [[ "$(cat "$foreign_home/.dainexus/mcp-server/sentinel")" == "foreign-runtime" ]] && \
...
test_round7_safety_boundaries() {
    ...
    if HOME="$safety_home" DAINEXUS_TEST_EXTERNAL_WRITE_AT=json-config \
        DAINEXUS_TEST_EXTERNAL_WRITE_CONTENT='{"external":true}' bash -c '
        source "$1"
        PROJECT_ROOT="$2"
        DAINEXUS_DIR="$3"
        write_json_mcp_config "$4" cursor
    ...
    if HOME="$safety_home" DAINEXUS_TEST_EXTERNAL_WRITE_AT=toml-config \
        DAINEXUS_TEST_EXTERNAL_WRITE_CONTENT=$'[external]\nvalue = "wins"\n' bash -c '
        source "$1"
        PROJECT_ROOT="$2"
        DAINEXUS_DIR="$3"
    ...
    if bash -c '
        ...
        node_version_is_supported v22.0.1
...
test_docs() {
    ...
    [[ -f "${DAINEXUS_DIR}/docs/SETUP.md" ]] && pass "SETUP.md exists" || fail "SETUP.md missing"
    [[ -f "${DAINEXUS_DIR}/docs/SETUP-QUICK.md" ]] && pass "SETUP-QUICK.md exists" || fail "SETUP-QUICK.md missing"
...
test_round8_fixes() {
    ...
    local settings_env="${TEST_PROJECT}/.dainexus/settings.env"
    ...
    local foreign_server="$HOME/.dainexus/mcp-server"
    ...
    local marker_file="$foreign_server/.dainexus-installation-owner.json"
    local sha256
...
EOF
    ...
    python3 - "$toml_target" "$DAINEXUS_LEDGER_PATH" "$DAINEXUS_RUNTIME_TOKEN" <<'PY'
import hashlib, json, os, sys, tomllib
target, ledger_path, token = sys.argv[1:]
target = os.path.realpath(target)
with open(target, "rb") as handle:
    entry = tomllib.load(handle)["mcp_servers"]["dai-nexus"]
key = hashlib.sha256(f"{target}:codex:dai-nexus".encode()).hexdigest()
fingerprint = hashlib.sha256(json.dumps(entry, sort_keys=True).encode()).hexdigest()
record = {"canonical_path": target, "schema": "codex", "managed_name": "dai-nexus",
          "created": True, "owned": True, "normalized_value_sha256": fingerprint}
with open(ledger_path, "w", encoding="utf-8") as handle:
    json.dump({"kind": "dai-nexus-mcp-ledger", "version": 1, "runtime_token": token,
...
test_snapshot_toctou_race() {
    ...
    # The setup script will attempt to add "dai-nexus" to the config.
    # While it is snapshotting, we will replace the file as a noncooperative writer.
    export DAINEXUS_TEST_INJECT_SNAPSHOT_RACE_PATH="$TEST_PROJECT/race-content"
    ...
    if grep -Fq '"hacker"' "$config_file" && ! grep -Fq '"dai-nexus"' "$config_file"; then
    ...
    unset DAINEXUS_TEST_INJECT_SNAPSHOT_RACE_PATH
    unset DAINEXUS_TEST_INJECT_SNAPSHOT_RACE_TARGET
}
...
${BLUE}▶ Testing Durable Client-Config Ownership Ledger${NC}"
    ...
    local ledger_path="$TEST_HOME/.dainexus/.mcp-config-ledger.json"
...
assert ledger["kind"] == "dai-nexus-mcp-ledger"
...
PY
    ...
    runtime_before="$(tree_digest "$TEST_HOME/.dainexus/mcp-server")"
...
PY
    ...
    if bash "$SCRIPT_DIR/dainexus-mcp-setup.sh" --uninstall >/dev/null 2>&1; then
        ...
        [[ "$runtime_before" != "$(tree_digest "$TEST_HOME/.dainexus/mcp-server")" ]]; then
...
main() {
    ...
    operator_state="$(python3 - "$OPERATOR_HOME/.dainexus/mcp-server" "$OPERATOR_HOME/.cursor/mcp.json" <<'PY'
...
PY
    ...
    echo ""
    echo -e "${CYAN}╔══════════════════════════════════════════════════╗${NC}"
    echo -e "${CYAN}║  DAI Nexus MCP Setup Test Suite              ║${NC}"
    echo -e "${CYAN}╚══════════════════════════════════════════════════╝${NC}"
    ...
    local current_state
    current_state="$(python3 - "$OPERATOR_HOME/.dainexus/mcp-server" "$OPERATOR_HOME/.cursor/mcp.json" <<'PY'
import os, hashlib, sys, json
def snapshot(path):
    if not os.path.lexists(path): return "missing"
    if os.path.islink(path): return "symlink"
