...
EXPORT_SCRIPT="$DAINEXUS_DIR/scripts/export-memory-diagnostic.sh"
...
mkdir -p "$tmp_workspace/.dainexus/offload/session-a/refs"
mkdir -p "$tmp_workspace/.dainexus/audit"
mkdir -p "$tmp_workspace/.dainexus/subagent-context"
...
cat > "$tmp_workspace/.dainexus/memory-bank/scenarios/scenario-a.md" <<'EOF'
# Scenario

password=supersecret
EOF

cat > "$tmp_workspace/.dainexus/offload/session-a/events.jsonl" <<'EOF'
{"node_id":"n1","token":"mytoken123456","summary":"event secret=hiddenvalue"}
EOF

cat > "$tmp_workspace/.dainexus/offload/session-a/state.json" <<'EOF'
{"session_id":"session-a","credential":"credential-secret"}
EOF

cat > "$tmp_workspace/.dainexus/offload/session-a/canvas.mmd" <<'EOF'
flowchart TD
  n1["done"]
EOF

cat > "$tmp_workspace/.dainexus/offload/session-a/refs/n1.md" <<'EOF'
raw ref contains token=rawsecrettoken
EOF

cat > "$tmp_workspace/.dainexus/audit/audit.jsonl" <<'EOF'
{"args":{"password":"auditpass"},"summary":"Bearer eyJhbGciOiJIUzI1NiJ9abcdef"}
EOF

cat > "$tmp_workspace/.dainexus/settings.env" <<'EOF'
API_TOKEN=settingssecret
EOF

cat > "$tmp_workspace/.dainexus/session-log.json" <<'EOF'
