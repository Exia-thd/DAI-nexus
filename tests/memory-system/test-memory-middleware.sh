...
# T4: memory stats in status
echo ""
echo "T4: memory stats in status output"
TESTS=$((TESTS+1))
output=$(python3 "$MIDDLEWARE" status 2>&1)
if echo "$output" | grep -qi "memory\|Memory Stats\|Observations"; then
    pass "memory stats shown"
else
    fail "memory stats missing"
fi
