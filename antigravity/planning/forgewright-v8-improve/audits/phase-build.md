...
| QG-03 | P1 | **Regression baseline** — `.dainexus/baseline-{session}.json` required for brownfield | Ensure baseline captured at session start |
| QG-04 | P2 | **Change manifest format** — `.dainexus/change-manifest-{session}.json` | Add schema validation |
...
| Finding | Severity | Evidence | Recommendation |
|---------|----------|----------|---------------|
| BS-01 | P1 | **Baseline captured per session** — `.dainexus/baseline-{session}.json` | Document baseline lifecycle |
