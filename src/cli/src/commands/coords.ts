...
async function handleConvert(
  ...
  if (options.json) {
    ...
    console.log(
      `    ${pc.dim("To:  ")} ${pc.green(formatPosition(convertedPos))}`,
    );
    ...
    if (rotation && convertedRot) {
      ...
      console.log(
        `    ${pc.dim("To:  ")} ${pc.green(formatRotation(convertedRot))}`,
      );
    }
    ...
    console.log(
      `    ${pc.dim("To:  ")} ${pc.green(formatScale(convertedScale))}`,
    );
...
async function handleValidate(
  ...
  if (options.json) {
    ...
    console.log();
    console.log(
      `  ${result.valid ? pc.green("✓ VALID") : pc.red("✗ INVALID")}`,
    );
    console.log();
  }
