---
description: Check for and install DAI Nexus updates
---
...
```bash
git -C dai-nexus fetch origin main
```

3. Check if there are new commits:
```bash
git -C dai-nexus log HEAD..origin/main --oneline
```

4. If new commits exist, update the submodule:
```bash
git submodule update --remote dai-nexus
```
...
```bash
git add dai-nexus
git commit -m "chore: update DAI Nexus to $(cat dai-nexus/VERSION)"
```
