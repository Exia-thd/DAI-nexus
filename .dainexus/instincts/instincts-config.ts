 ...
 * 
 * Controls:
 * - Enable/disable entire system (DAINEXUS_INSTINCTS_ENABLED=0)
...
export const ENV_VARS = {
  ENABLED: 'DAINEXUS_INSTINCTS_ENABLED',
  CONFIG_PATH: 'DAINEXUS_INSTINCTS_CONFIG',
  STORE_PATH: 'DAINEXUS_INSTINCTS_STORE',
  LOG_LEVEL: 'DAINEXUS_INSTINCTS_LOG',
} as const;
...
const DEFAULT_CONFIG: InstinctsConfig = {
  ...
  storePath: resolve(process.env.DAINEXUS_DIR || '', '.dainexus/instincts/store.json'),
  maxPatterns: 1000,
  pruneThreshold: 0.15,
  logLevel: 'info',
  autoSuggest: true,
...
export function getInstinctsConfig(overrides?: Partial<InstinctsConfig>): InstinctsConfig {
  ...
  const configPath = process.env[ENV_VARS.CONFIG_PATH] || 
    resolve(process.env.DAINEXUS_DIR || '', '.dainexus/instincts-config.json');
