...
Assess whether TencentDB-Agent-Memory has ideas that can upgrade DAI Nexus.
...
1. DAI Nexus has long-term memory layering, but does not appear to have Tencent-style short-term context offload with a live Mermaid task canvas.
...
1. Short-term context offload canvas for MCP tool calls.
   - Persist full tool output under `.dainexus/offload/<session>/refs/`.
...
2. Drill-down memory CLI.
   ...
   - Tie MCP audit refs to memory graph nodes.

3. Scenario/persona layer on top of memory.
   - Materialize `.dainexus/memory-bank/scenarios/*.md` and `.dainexus/memory-bank/persona.md`.
...
- DAI Nexus currently supports broader Codex/Cursor/Claude/OpenCode workflows; the right path is adapting concepts, not installing the plugin.
