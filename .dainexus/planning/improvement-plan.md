# DAI Nexus Improvement Plan — Post-ECC Review

**Date:** 2026-06-02
**Source:** ECC vs DAI Nexus Comparative Review
**Plan Quality Score:** 7.4/10 (v1) → Target: ≥ 9.0/10
**Status:** IN PROGRESS — awaiting HIGH priority fixes
**Owner:** DAI Nexus Core Team

---

## Strategic Framing

DAI Nexus has a **structural advantage** ECC lacks: canonical MCP server at `~/.dainexus/mcp-server/` — one server, all platforms. ECC uses per-harness TOML/JSON configs that drift. This is the right architectural bet.
...
|----|-------|------|------------|--------|------------|
| R1 | All | memory v2 API breaking change | Low | High | Pin to stable version, wrapper abstraction layer |
| R2 | 2 | AgentShield wraps ECC — external repo dependency | Medium | Medium | Fork AgentShield into dai-nexus-security, maintain separately |
...
| R8 | 1 | Instinct System conflicts with existing memory patterns | Medium | Medium | Run both systems in parallel for 2 sprints, compare, then consolidate |
...
**Value:** DAI Nexus learns continuously — no manual "save this pattern"
**Dependencies:** Hook system (existing), memory-v2 (existing)
...
**What to build:** Wrap ECC's AgentShield or fork into dai-nexus-security.

**Implementation:**
```yaml
# Option A (RECOMMENDED): Fork into dai-nexus-security
.dainexus/security/
...
**Effort:** Medium — 5 days
**Risk Mitigation:** Fork AgentShield into `.dainexus/security/agent-shield/` — do not depend on external ECC repo directly
**Acceptance Criteria:**
...
**Implementation:**
```bash
# In .env or dai-nexus config
DAINEXUS_HOOK_PROFILE=minimal|standard|strict
DAINEXUS_DISABLED_HOOKS=memory-tick,token-check
DAINEXUS_SESSION_START_MAX_CHARS=50000
```

```typescript
// hooks/index.ts
const profile = process.env.DAINEXUS_HOOK_PROFILE ?? 'standard';
const disabled = process.env.DAINEXUS_DISABLED_HOOKS?.split(',') ?? [];
...
- [ ] CI/CD can set `DAINEXUS_HOOK_PROFILE=minimal` to minimize overhead
...
**Value:** DAI Nexus reaches Gemini, Zed, OpenCode users — same canonical server.
...
**What to build:**
```bash
dai-nexus install --profile minimal   # Only core pipeline
dai-nexus install --profile core      # + security, QA
dai-nexus install --profile full      # Everything

dai-nexus uninstall --repair          # Fix broken install
dai-nexus doctor                      # Diagnose issues
dai-nexus list-installed              # Show what's installed
```
...
- [ ] `dai-nexus install --profile full` installs everything
- [ ] `dai-nexus doctor` reports health of all installed components
- [ ] `dai-nexus uninstall` removes all DAI Nexus configs cleanly
...
**Value:** Non-technical users can explore DAI Nexus capabilities visually.
