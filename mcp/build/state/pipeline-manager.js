...
let _dai-nexusRoot = null;
let _workspaceRoot = null;
function _getDaiNexusRoot() {
    if (!_dai-nexusRoot) {
        let dir = __dirname;
        for (let i = 0; i < 10; i++) {
...
export function getDaiNexusRoot() {
...
export function _isResolvedPath(p) {
    if (!p)
        return false;
    if (/\$\{[^}]+\}/.test(p)) {
        console.error(`[DAI Nexus Global MCP] Warning: Skipping unresolved template variable in path: "${p}"`);
        return false;
    }
    return true;
}
export function setWorkspaceRoot() {
    ...
    if (!ws) {
        console.error(`[DAI Nexus Global MCP] Warning: Could not detect workspace. Using DAINEXUS_ROOT.`);
        ws = _getDaiNexusRoot();
    }
